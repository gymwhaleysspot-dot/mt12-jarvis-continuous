...
function p219SceneSync(reason='LIVE'){
 ...
 const stage=Number(scene.stage)||1;if(stage!==p219LastStage){
  ...
  p219LastStage=stage;p219LastEpoch=Number(p218SceneRuntime?.epoch)||0;production219.sceneRebinds++;
 }
...
function p219FormIndex(){const f=String(typeof currentFormName==='function'?currentFormName():griffin?.form||'').toUpperCase();if(!f)return 0;const names=Array.isArray(forms)?forms:[];const i=names.findIndex(x=>String(x?.name||x||'').toUpperCase()===f);return i>=0?i:Math.max(0,Number(griffin?.formIndex)||0)}
