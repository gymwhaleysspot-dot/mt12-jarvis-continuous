...
const formAtlases130=[atlas119.griffin,...['saiyan-spark','ascended','hyper','instinct'].map(name=>{const image=new Image(),model={name,image,ready:false};image.onload=()=>model.ready=true;image.onerror=()=>model.ready=false;image.src=`jarvis/assets/survivor/griffin-${name}-atlas-v1.webp`;return model})];
...
const seq128={IDLE:[0,1,0,2],STEP:[1,2,3,2],DASH:[2,3,2,1],GUARD:[4,0,4,1],JAB:[0,5,6,1],CROSS:[1,6,5,0],KICK:[2,6,5,1],SPIN:[3,6,2,0],BEAM:[4,5,6,4],HIT:[6,4,0,1],RECOVER:[3,2,1,0]};
...
function pose128(entity,pose,seed=0){const now=performance.now(),s=entity===player?stage127.hero:(stage128.enemy.get(entity)||{});if(s.pose!==pose){s.pose=pose;s.started=now;production128.poseChanges++}if(!s.started)s.started=now;const q=seq128[pose]||seq128.GUARD,step=/JAB|CROSS|KICK|SPIN|BEAM|HIT/.test(pose)?78:108,index=Math.floor((now-s.started+seed*23)/step)%q.length;if(entity!==player)stage128.enemy.set(entity,s);return{frame:q[index],phase:index/Math.max(1,q.length-1)}}
...
function reveal128(g,now,base,ground,flip){const tr=griffin.transformation,form=clamp(tr?.stage??griffin.evolution??0,0,4);if(tr?.active&&stage128.revealStage!==form){stage128.revealStage=form;stage128.revealUntil=now+1450;production128.reveals++}if(now>=stage128.revealUntil)return false;const spec=saiyanForms[form],q=clamp((stage128.revealUntil-now)/1450,0,1),w=base*(.78+.05*Math.sin((1-q)*Math.PI));g.save();g.fillStyle=`rgba(2,3,10,${.58+.22*q})`;g.fillRect(0,0,W,H);g.restore();shadow128(g,W*.5,ground,w,.52);flameAura127(g,W*.5,ground,w*1.08,spec.color,1.25);if(forms127Ready)formHero127(g,form,W*.5,ground,w,flip,'GUARD',1-q);g.save();g.textAlign='center';g.shadowBlur=22;g.shadowColor=spec.color;g.fillStyle=spec.color;g.font=`900 ${Math.max(21,Math.min(45,W*.052))}px system-ui`;g.fillText(spec.name,W*.5,H*.2);g.shadowBlur=0;g.fillStyle='#efffff';g.font=`800 ${Math.max(9,Math.min(15,W*.016))}px system-ui`;g.fillText('FORM REVEALED · COMBAT RIG ENGAGED',W*.5,H*.255);g.restore();return true}
...
const liraForms131=[
 {name:'LIRA VANGUARD',color:'#ff416c',src:'lira-vanguard-atlas-v1.webp'},
 {name:'LIRA RIFT',color:'#a94dff',src:'lira-rift-atlas-v1.webp'},
 {name:'LIRA ECLIPSE',color:'#ff62d7',src:'lira-eclipse-atlas-v1.webp'},
 {name:'NEXUS EMPRESS',color:'#ffb0dd',src:'lira-empress-atlas-v1.webp'}
];
...
const seq131={IDLE:[0,1,0,2],STEP:[1,2,1,0],DASH:[2,3,2,1],GUARD:[4,4,0,4],JAB:[8,8,0],CROSS:[9,9,0],KICK:[10,10,0],SPIN:[11,11,3],BEAM:[12,12,4],HIT_LIGHT:[5,5,0],HIT_MEDIUM:[6,6,15],HIT_HEAVY:[7,7,13],LAUNCH:[13,13,14],KNOCKDOWN:[14,14,15],RECOVER:[15,15,4]};
...
function liraReveal131(g,now,ground,base){const r=stage131.liraReveal;if(!r||now>=r.until)return false;const model=liraAtlases131[r.form],q=clamp((r.until-now)/1250,0,1);g.save();g.fillStyle=`rgba(3,1,12,${.48+.3*q})`;g.fillRect(0,0,W,H);g.restore();if(model.ready){shadow128(g,W*.5,ground,base,.54);cel125(g,model.image,0,W*.5,ground,base,false,1,'GUARD',0);production131.modelDraws[r.form]++}g.save();g.textAlign='center';g.shadowBlur=24;g.shadowColor=model.color;g.fillStyle=model.color;g.font=`900 ${Math.max(22,Math.min(48,W*.055))}px system-ui`;g.fillText(model.name,W*.5,H*.21);g.shadowBlur=0;g.fillStyle='#fff1fb';g.font=`800 ${Math.max(10,Math.min(16,W*.018))}px system-ui`;g.fillText('LIRA TRANSFORMATION · NEW BODY ONLINE',W*.5,H*.265);g.restore();return true}
function render131(){const now=performance.now();if(now-stage131.lastPaint<24)return;stage131.lastPaint=now;vector113Boot();const cv=vector113.cv,g=vector113.g,d=Math.min(devicePixelRatio||1,W<720?1.25:1.5),ww=Math.max(1,W*d|0),hh=Math.max(1,H*d|0);if(cv.width!==ww||cv.height!==hh){cv.width=ww;cv.height=hh}g.setTransform(d,0,0,d,0,0);g.clearRect(0,0,W,H);arena119(g);const base=Math.min(W,H)*(W<720?.55:.37),ground=H*.855,target=griffin.target&&enemies.includes(griffin.target)?griffin.target:null,dir=target&&target.x<player.x?-1:1,form=clamp(griffin.evolution||0,0,4),color=saiyanForms[form].color;if(liraReveal131(g,now,ground,base)){combatOverlay117(g);production131.frames++;return}const desiredHero=W*.5-dir*base*.13+Math.sin(now*.003)*base*.035;stage131.heroX=clamp(stage131.heroX+(desiredHero-stage131.heroX)*.16,base*.48,W-base*.48);const near=enemies.filter(e=>e&&Number.isFinite(e.x)&&Number.isFinite(e.y)).sort((a,b)=>Math.hypot(a.x-player.x,a.y-player.y)-Math.hypot(b.x-player.x,b.y-player.y)).slice(0,W<720?1:2),positions=[];for(const [i,e]of near.entries()){const modelIndex=liraForm131(e),model=liraAtlases131[modelIndex],size=base*(e.type===3?.94:.67),side=i===0?dir:-dir,s=liraState131(e,now),desired=stage131.heroX+side*base*(i===0?.88:1.18),minX=size*.48,maxX=W-size*.48,x=clamp(Number.isFinite(s.x)?s.x+(desired-s.x)*.14:desired,minX,maxX);s.x=x;positions.push(x);const fallback=e.attackClock<.24?['JAB','CROSS','KICK','SPIN'][i%4]:e.role==='RUSHER'?'DASH':i%2?'STEP':'GUARD',a=pose131(s,now,fallback,29+i*43),age=now-s.started,p=clamp(age/Math.max(1,s.reaction||1),0,1),launch=a.pose==='LAUNCH'?Math.sin(Math.PI*p)*size*.3:a.pose==='KNOCKDOWN'?size*.02:0,knock=/HIT_HEAVY|LAUNCH/.test(a.pose)?side*Math.sin(Math.PI*Math.min(1,p))*size*.16:0;shadow128(g,x+knock,ground,size,a.pose==='KNOCKDOWN'?.18:.36);if(model.ready){cel125(g,model.image,a.frame,x+knock,ground-launch,size,x<stage131.heroX,1,a.pose,a.phase);production131.modelDraws[modelIndex]++}if(stage131.impact&&stage131.impact.x===0){stage131.impact.x=clamp((x+stage131.heroX)*.5,22,W-22);stage131.impact.y=ground-size*.48}}
 const heroLoss=Math.max(0,stage131.hero.hp-player.hp);stage131.hero.hp=player.hp;if(heroLoss>.1){stage131.hero.until=now+clamp(170+heroLoss*9,210,520);stage131.hero.started=now;production131.heroReactions++;production131.hitStops++}let heroPose=now<stage131.hero.until?'HIT':director127(now,true,0,false),ha=pose128(player,heroPose,0),px=positions[0]??stage131.heroX+dir*base,heroModel=formAtlases130[form]?.ready?formAtlases130[form].image:atlas119.griffin,heroKnock=now<stage131.hero.until?-dir*Math.sin(Math.PI*clamp((now-stage131.hero.started)/(stage131.hero.until-stage131.hero.started),0,1))*base*.12:0;shadow128(g,stage131.heroX+heroKnock,ground,base,.46);if(form)flameAura127(g,stage131.heroX+heroKnock,ground,base*.9,color,.25);if(heroModel&&heroModel.complete)atlasSafe127(g,heroModel,ha,stage131.heroX+heroKnock,ground,base,dir<0,heroPose,now<stage131.hero.until,0);if(stage131.impact&&stage131.impact.x===0){stage131.impact.x=clamp((px+stage131.heroX)*.5,22,W-22);stage131.impact.y=ground-base*.45}impact131(g,stage131.impact,now);combatOverlay117(g);const primary=near[0],ps=primary&&stage131.enemy.get(primary);if(ps?.combo>1&&now-ps.lastHit<900){g.save();g.textAlign='center';g.fillStyle='#fff';g.shadowBlur=15;g.shadowColor=liraForms131[liraForm131(primary)].color;g.font=`900 ${Math.max(12,Math.min(24,W*.03))}px system-ui`;g.fillText(`${ps.combo} HIT PRESSURE`,W*.5,H*.34);g.restore()}production131.frames++}
