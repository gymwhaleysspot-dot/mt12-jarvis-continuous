...
function render128(){const now=performance.now();if(now-stage128.lastPaint<24)return;stage128.lastPaint=now;vector113Boot();const cv=vector113.cv,g=vector113.g,d=Math.min(devicePixelRatio||1,W<720?1.25:1.5),ww=Math.max(1,W*d|0),hh=Math.max(1,H*d|0);if(cv.width!==ww||cv.height!==hh){cv.width=ww;cv.height=hh}g.setTransform(d,0,0,d,0,0);g.clearRect(0,0,W,H);arena119(g);const ready=atlas119.gReady&&atlas119.lReady,base=Math.min(W,H)*(W<720?.55:.37),ground=H*.855,target=griffin.target&&enemies.includes(griffin.target)?griffin.target:null,dir=target&&target.x<player.x?-1:1,form=clamp(griffin.evolution||0,0,4),color=saiyanForms[form].color,combatPose=director127(now,true,0,false),motion=/DASH|STEP/.test(combatPose)?Math.sin(now*.012)*base*.075:/JAB|CROSS|KICK|SPIN/.test(combatPose)?dir*base*.055:0,heroWant=W*.5-dir*base*.13+motion;stage128.heroX=clamp(stage128.heroX+(heroWant-stage128.heroX)*.17,base*.48,W-base*.48);if(reveal128(g,now,base,ground,dir<0)){combatOverlay117(g);production128.frames++;return}const near=enemies.filter(e=>e&&Number.isFinite(e.x)&&Number.isFinite(e.y)).sort((a,b)=>Math.hypot(a.x-player.x,a.y-player.y)-Math.hypot(b.x-player.x,b.y-player.y)).slice(0,W<720?1:2),positions=[];for(const [i,e]of near.entries()){const size=base*(e.type===3?.9:.66),side=i===0?dir:-dir,s=stage128.enemy.get(e)||{},variant=(e.id??i)%3,desired=stage128.heroX+side*base*(i===0?.9:1.18)+Math.sin(now*.003+(e.id||i))*base*.055,minX=size*.48,maxX=W-size*.48;let x=clamp(Number.isFinite(s.x)?s.x+(desired-s.x)*(.13+variant*.025):desired,minX,maxX);if(Math.abs(x-stage128.heroX)<base*.72)x=clamp(stage128.heroX+side*base*.72,minX,maxX);s.x=x;stage128.enemy.set(e,s);positions.push(x);const poses=['GUARD','STEP','DASH'],ep=e.hit>0?'HIT':(director127(now+variant*170,false,i,e.hit>0)||poses[variant]),a=pose128(e,ep,53+i*67+variant*19),bob=Math.sin(now*.009+variant*2.1)*size*.012;shadow128(g,x,ground,size,.34);const out=ready?atlasSafe127(g,atlas119.lira,a,x,ground+bob,size,x<stage128.heroX,ep,e.hit>0,0):null;production128.enemyVariants++;if(out?.contact&&Math.abs(x-stage128.heroX)<base*.98&&now-stage128.lastImpact>270){stage128.lastImpact=now;stage128.impact={start:now,x:clamp((out.x+stage128.heroX)*.5,18,W-18),y:ground-size*.42,color:'#ff477e',enemy:e};production128.combatFrames++}}
 const primary=near[0],px=positions[0]??stage128.heroX+dir*base,a=pose128(player,combatPose,0),bob=/STEP|DASH/.test(combatPose)?Math.abs(Math.sin(a.phase*TAU))*base*.022:0;shadow128(g,stage128.heroX,ground,base,.46);if(form)flameAura127(g,stage128.heroX,ground,base*.9,color,.62);const out=ready?atlasSafe127(g,atlas119.griffin,a,stage128.heroX,ground-bob,base,dir<0,combatPose,false,0):null;formIdentity128(g,stage128.heroX,ground-bob,base,dir<0,form,combatPose,a.phase);if(out?.contact&&Math.abs(px-stage128.heroX)<base*.99&&now-stage128.lastImpact>270){stage128.lastImpact=now;stage128.impact={start:now,x:clamp((out.x+px)*.5,18,W-18),y:ground-base*.43,color,enemy:primary};production128.combatFrames++}impact125(g,stage128.impact,now);combatOverlay117(g);production128.frames++}
