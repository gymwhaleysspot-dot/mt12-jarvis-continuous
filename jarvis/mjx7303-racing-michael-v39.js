...
let polyglot=null;bootMichaelPolyglot('./generated/','michael84').then(api=>{polyglot=api;globalThis.__MICHAEL_POLYGLOT=api}).catch(error=>console.warn('MICHAEL V59 optional module fallback',error));
...
function refineVehicle(){
  const ds=xr?.drawables||[],same=(m,v,t=.002)=>!!m?.base&&Math.abs(m.base[0]-v[0])<t&&Math.abs(m.base[1]-v[1])<t&&Math.abs(m.base[2]-v[2])<t;
  let projectors=0,wheels=0;
  for(const d of ds){const m=d.mat;
    // V59 keeps the identity/livery batches visible. Only normalize legacy optical material and wheel finish.
  ...
  globalThis.__MICHAEL_REFINEMENT={legacyOverlayHidden:0,projectorsDarkened:projectors,whiteRallyWheelBatches:wheels,identityBatchesPreserved:true,trackPose:true,cabinPreserved:true,carbonPreserved:true,steeringSign:'DIRECT_X',bodyAuthority:'OWNER_7303_PHOTOS'};
}
// Nickelle may tune bounded materials/proportions, but V59 no longer appends a second body-authority mesh.
globalThis.__MICHAEL_NATIVE_VISUAL={wheelRadius:0.512,wheelTrackX:1.37,wheelWidth:0.43,wheelY:0.56,paintRoughnessScale:1,darkRoughnessScale:1,exposureScale:1,keyLightScale:1,ambientScale:1,headlampTopology:'OWNER_ELONGATED_RECT_V59',grilleTopology:'OWNER_LOW_WIDE_MESH_V59',rearLampTopology:'OWNER_LARGE_ROUNDED_V59',generation:5};
