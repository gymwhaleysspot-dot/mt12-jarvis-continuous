(()=>{'use strict';
const H='survivor',SRC='survivor.html?v=20260814production105';let observer,timer;
if(!document.querySelector('link[data-survivor-style]')){const link=document.createElement('link');link.rel='stylesheet';link.href='jarvis/survivor-tab.css?v=20260814production105';link.dataset.survivorStyle='1';document.head.append(link)}
...
function mount(){
 ...
 const frame=body.querySelector('.survivor-frame');if(frame?.src.includes('20260814production105'))return;
...
addEventListener('hashchange',()=>queueMicrotask(mount));addEventListener('pageshow',()=>{watch();mount()});
if(document.readyState==='loading')addEventListener('DOMContentLoaded',()=>{watch();mount()},{once:true});else{watch();mount()}
})();
