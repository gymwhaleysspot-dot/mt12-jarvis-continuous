(()=>{'use strict';
const H='survivor',SRC='survivor.html?v=20260819scratch300balance2';let observer,timer;
if(!document.querySelector('link[data-survivor-style]')){const link=document.createElement('link');link.rel='stylesheet';link.href='jarvis/survivor-tab.css?v=20260819scratch300balance2';link.dataset.survivorStyle='1';document.head.append(link)}
...
function mount(){
 ...
 document.getElementById('workspaceDescription').textContent='Griffin climbs a nine-round ascension tournament through one canonical sprite-driven adaptive combat runtime.';
 ...
 const frame=body.querySelector('.survivor-frame');if(frame?.src.includes('20260819scratch300balance2'))return;
 body.innerHTML=`<section class="survivor-card"><div class="survivor-load">Initializing adaptive Survivor combat…</div><iframe class="survivor-frame" src="${SRC}" title="Jarvis Core Survivor" allow="autoplay; fullscreen" loading="eager"></iframe></section>`;
