...
export async function bootMichaelPolyglot(_base = './generated/', cache = 'michael74') {
    const states = [];
    let cpp = null, rust = null, assembly = null, zig = null;
    let fallbackExposure = 1.18;
    try {
        const factory = (await import(assetUrl('michael-core.js', cache))).default;
        cpp = await factory({ locateFile: (n) => assetUrl(n, cache) });
    ...
    for (const [name, file, set] of [['Rust Wasm', 'michael-rust.wasm', (v) => rust = v], ['AssemblyScript Wasm', 'michael-assembly.wasm', (v) => assembly = v], ['Zig Wasm', 'michael-zig.wasm', (v) => zig = v]]) {
        try {
            const v = (await instantiate(assetUrl(file, cache))).instance.exports;
            set(v);
    ...
    const cameraPreset = (view, aspect) => { const portrait = Math.max(0, Math.min(1, (1 - Math.max(.45, Math.min(1.8, aspect))) / .55)), fit = 1 + portrait * .015, presets = { front: [0, .050, 9.80, .50], three: [-.72, .105, 11.10, .55], side: [-Math.PI / 2, .050, 12.30, .53], rear: [Math.PI, .050, 9.80, .50], top: [-.42, .895, 17.20, .40] }, p = presets[view] || presets.three; return { yaw: p[0], pitch: p[1], distance: p[2] * fit, targetY: p[3] }; };
    void zig;
