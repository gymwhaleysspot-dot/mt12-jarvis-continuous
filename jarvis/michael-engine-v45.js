// MICHAEL ENGINE V47 — custom heavyweight browser game-engine kernel.
...
export class MichaelEngine{
  ...
  report(){return{version:this.version,backend:this.backend,webgpuAvailable:!!navigator.gpu,frame:this.frame,cpuMs:+this.cpuMs.toFixed(2),gpuMs:+this.profiler.gpuMs.toFixed(2),gpuTiming:this.profiler.supported,ownedRenderPasses:true,hdrTarget:this.renderer.hdrFloat?'RGBA16F':'RGBA8_FALLBACK',postProcess:true,tier:this.policy.tier,features:{...this.policy.features},renderGraph:this.graph.describe(),world:this.world.snapshot(),resources:this.resources.report(),uptimeMs:Math.round(performance.now()-this.started)}}
}
