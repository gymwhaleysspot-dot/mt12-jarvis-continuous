...
export async function syncMT12(inputs={}){
 ...
 const prior=state.data['public/evidence-data/sync.json']||{};
 state.data['public/evidence-data/sync.json']={...prior,state:'REQUESTED',stage:'dispatching',message:'MT12 evidence sync requested. Keep the radio connected to the Jarvis helper.',updatedAt:new Date().toISOString()};emit();
 await api.dispatch(workflow,{include_radio_logs:'true',include_blackbox:'true',backup_first:'true',start_learning:'true',...inputs});
 await refresh();
 const watch=async(n=0)=>{if(n>=20)return;const s=state.data['public/evidence-data/sync.json']||{};if(['IMPORTED','FAILED','ERROR'].includes(String(s.state||'').toUpperCase()))return;await new Promise(r=>setTimeout(r,3000));await refresh();return watch(n+1)};
 watch().catch(()=>{});return true
}
