...
local function loadController()local ok,m=pcall(dofile,"/home/runner/work/mt12-jarvis-continuous/mt12-jarvis-continuous/generations/candidates/0101-m101a.lua");if not ok then return nil,'load:'..tostring(m)end;if type(m)~='table'then return nil,'module-return'end;return m end
...
local j='{"schema":2,"engine":"official-edgetx-source-backed-mt12-emulator","edgetxCommit":"f8a89393c560a1057b045048f4b6ff5096ca22a0","candidate":"0101-m101a","passed":'..tostring(pass)..',"scenarios":[';for i,r in ipairs(results)do if i>1 then j=j..','end;j=j..'{"name":"'..esc(r.name)..'","passed":'..tostring(r.passed)..',"error":'..(r.error and('"'..esc(r.error)..'"')or'null')..',"tct":'..tostring(r.tct or 0)..',"gyr":'..tostring(r.gyr or 0)..',"lcd":'..tostring(r.lcd or 0)..',"sounds":'..tostring(r.sounds or 0)..',"wrote":'..tostring(r.wrote or false)..',"writeBytes":'..tostring(r.writeBytes or 0)..'}'end;j=j..']}'
local f=assert(realio.open("/home/runner/work/mt12-jarvis-continuous/mt12-jarvis-continuous/reports/0101-m101a-edgetx-emulator.json",'w'));f:write(j);f:close();print(j);if not pass then os.exit(2)end
