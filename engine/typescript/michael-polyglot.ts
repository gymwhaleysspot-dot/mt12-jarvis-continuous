...
export async function bootMichaelPolyglot(base='./generated/',cache='michael61'):Promise<MichaelPolyglot>{
  const states:MichaelModuleState[]=[]; let cpp:any=null,rust:any=null,assembly:any=null,zig:any=null; let fallbackExposure=1.9;
  ...
  const exposure=(drive:boolean,frameMs:number,dt:number)=>{if(cpp)return cpp._michael_visual_tick(drive?1:0,frameMs,dt);const target=(drive?2.18:1.94)-Math.max(0,Math.min(1,(frameMs-16.7)/24))*.1;fallbackExposure+=(target-fallbackExposure)*(1-Math.exp(-4.5*Math.min(.1,dt)));return fallbackExposure;};
