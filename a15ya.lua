--a15xz predictive causal arbiter evolution
...
local m_pi=math.pi
local function clamp(x,a,b)x=x or 0;a=a or 0;b=b or a;if x<a then return a elseif x>b then return b else return x end end;local function c9(x)return clamp(x,0,99)end;local function c1(x)return clamp(x,0,1)end;local function c0(x)return clamp(x,0,100)end
...
local function traitLearn()
 local clean=c9((V[333])/(m_max(1,V[289]))*100);local ev=c9(V[289]);local sev=c9(V[302]);local swing=c9((V[341])+(V[342])*1.5+(V[343]));local data=c9(V[347]);local mismatch=c9(V[304]);local speed=clamp(V[331],0,55)
 local commit=c9(42+speed*.75+clean*.20+(V[325])*.18-sev*.55-ev*.04)
 local compose=c9(52+clean*.45+data*.12-sev*.55-swing*.36)
 local adapt=c9(50+((V[239])-65)*.55+(data-80)*.20-mismatch*.18-(V[119])*16)
 local sty=c9((V[560])*.44+(V[561])*.30+(V[562])*.26)
 local note=c9(46+data*.38-mismatch*.30+((V[239])-65)*.25+clean*.10-sev*.12)
 V[370]=trait(V[370],commit,.014);V[371]=trait(V[371],compose,.014);V[372]=trait(V[372],adapt,.012);V[373]=trait(V[373],sty,.016);V[374]=trait(V[374],note,.012);V[375]=c9((V[370]+V[371]+V[372]+V[374])*.25);V[376]=i9(V[376])
end
local function wrmem()
 if memSaved>0 then return end;memSaved=1;local a={m_max(V[513],100-(V[239]),100-(V[602])),m_max(V[520],V[606]),m_max(V[503],V[262])-(V[501])*.2,100-(V[168])+(V[69])*20};local k=1;local h=a[1];local l=0;for i=2,4 do if a[i]>h then l=h;h=a[i];k=i elseif a[i]>l then l=a[i]end end;local cc=c9(h-l);if cc<12 or k==4 then V[638]=m_max(20,V[638]-6)elseif k~=3 then V[503]=V[503]*.85 end;V[405]=p2221(k,cc,V[520],(V[503])/10);if V[655]>0 and (V[541])>38 and (V[543])==0 then traitLearn();V[548]=0 else V[548]=1 end;local f=io.open(MEMPATH,"w");if not f then return end
...
local function voice(n,p,cd)
 ...
 if p<5 and vLoad>82 then voiceBlock=i9(voiceBlock);return end
 if p<4 and (V[550])>78 then voiceBlock=i9(voiceBlock);return end
 ...
 vcLast=n;vcPr=p;vcCd=cd;vcCnt=i9(vcCnt);V[570]=n;V[571]=vcCnt;V[572]=p;V[573]=cd/10;vLoad=c9(vLoad+p*5);return n
end
local function voiceTick()
 if vcCd>0 then vcCd=vcCd-1;if vcCd==0 then vcPr=0 end end
 vLoad=c9(vLoad*.985);vQuiet=clamp(50+vLoad*.45+(V[550])*.20+(V[386])*.10,50,99)
...
local function cue(n,p,cd)
 p=p or 2;cd=cd or 90;if V[654]==0 and p<4 then sndBlock=i9(sndBlock);return end;if p<4 and((V[36])>12 or(V[37])>10)then sndBlock=i9(sndBlock);return end;if sndCd>0 and(p<sndPr or n==sndLast)then sndBlock=i9(sndBlock);return end;if not voice(n,p,cd*2)then return end;sndCd=cd;sndPr=p;sndLast=n;V[550]=c9((V[550])+p*2);return n
end
...
local function na(x)return c1(.5+x*.25)end
...
local function ol(p,o,v,n)local k=670+p;V[k]=clamp((V[k]>0 and V[k])+(o>0 and 1 or-1),20,80);V[680]=p;V[681]=o;V[682]=c9(v or 0);V[683]=n or 0;if n and n>0 then vl(n,o>0 and 1 or-1)end end
...
local c=V[59];if c==0 then c=4 end;local raw=c0(rx/(c*4.2)*100);V[57]=V[57]+(raw-V[57])*0.02;setgv(5,V[57]);return m_floor(V[57]+0.5)
end
...
local ms=mph*.44704;local yr=(ms<.5)and 0 or ms*m_tan(clamp(st/1024,-1,1)*MAX_STEER*m_pi/180)/WB;V[13]=V[13]*.9+yr*.1;V[12]=(ms<1)and 0 or clamp(m_atan(V[13]*WB/ms)*57.2958,-30,30);local ay=m_abs(V[13]);yT=c1(yT+(((ay-yZ)>.02 and th>180 and m_abs(st)>140)and .12 or -.04));yZ=ay
...
local jp=V[620];local vv=V[119]*100;local air=c1((V[410]-18)*.05+(V[423]-V[425])*.035+m_max(V[421],0)*.22+flare*.18-md*.16);local grd=c1((V[426]-24)*.045+m_max(-V[421],0)*.24+(vv-X[10])*.018+(rM<mph+3 and .16 or 0));local aa=clamp(X[18]+(air-.42)*1.35,0,9);local gg=clamp(X[19]+(grd-.38)*1.3,0,9);if air<.22 then aa=m_max(0,aa-.55)end;if grd<.2 then gg=m_max(0,gg-.5)end;X[18]=aa;X[19]=gg;V[427]=aa*11;V[428]=gg*11;if jp==0 then if vv<22 then X[12]=1 end;if X[12]>0 and aa>=2.6 and air>grd+.08 then V[620]=1;V[604]=1;X[8]=X[8]%99+1;X[9]=0;X[19]=0 end elseif jp==1 then V[604]=V[604]+1;if aa>=4.7 and air>.48 then V[620]=2;V[621]=12 elseif V[604]>9 or aa<.8 then X[9]=1;V[620]=0;V[604]=0 end elseif jp==2 then V[604]=V[604]+1;if gg>=3.1 and grd>.4 then X[9]=3;V[620]=3;V[621]=18;X[10]=vv;X[12]=0 elseif V[604]>20 then X[9]=2;V[620]=4;V[621]=9;X[10]=vv;X[12]=0 end else X[10]=m_min(X[10],vv);if X[10]<22 and vv-X[10]>16 and vv-X[11]>2 then X[12]=1 end;if jp==3 and X[12]>0 and aa>=2.8 and air>grd+.06 then V[620]=1;V[604]=1;X[8]=X[8]%99+1;X[9]=0;X[12]=0;X[19]=0 elseif V[621]>0 then V[621]=V[621]-1 else V[620]=0;V[604]=0;X[18]=aa*.55;X[19]=gg*.55 end end;X[11]=vv;if V[620]~=jp then if(jp==1 and V[620]==0)or(jp==2 and V[620]>=3)then X[24]=p2221(X[8],9,X[9],(V[38]>0 and 1 or 0)+(V[39]>0 and 2 or 0))elseif not(jp>=3 and V[620]==0)then X[24]=p2221(X[8],jp,V[620],X[9])end;X[25]=p2221(c9(md*100),c9(V[119]*100),c9(j),clamp(l/10,0,9))end;if V[620]>0 and V[620]~=3 then V[603]=m_max(V[603],14);V[409]=m_max(0,V[409]-2)end;updateClassifier(mph,dSt,V[16]>0)
...
if ph>0 then if V[600]==0 then V[614]=mph;V[615]=0;V[616]=0;V[613]=(st<0)and 1 or 2 end;V[615]=m_max(V[615],slip);if V[38]>0 or V[39]>0 then V[616]=1 end elseif V[600]>0 and V[603]==0 and V[604]==0 and V[627]>0 then local z=c9(50+(mph-V[614])*4-(V[615])*2-(V[616])*12);V[601]=z;V[602]=trait(V[602],z,.01);V[(V[613]==1)and 607 or 608]=trait(V[(V[613]==1)and 607 or 608],z,.012);V[610]=trait(V[610],100-m_abs(z-50)-V[606]*.2,.01);if V[616]==0 then V[611]=i9(V[611]);V[612]=trait(V[612],z,.015)end end
...
local counter=(V[16]>0 and m_abs(V[12])>3 and st*V[12]<0);if counter then smart=smart*(V[108]/100)end;smart=smart+(50-V[601])*.05+V[409]*.035-V[410]*.025+(tr and(50-(st<0 and V[607]))*.04 or 0)+aiNext*5+(V[603]>0 and 3 or 0)+(V[625]>70 and 4 or 0);if V[628]>70 then smart=smart-3 end;if(V[604]>0 and V[604]<4)or V[620]==3 then smart=m_max(smart,145)end
...
V[66]=c1((th>0 and th or 0)/1024*steerLoad);V[67]=brake and steerLoad or 0;if brake then V[245]=i9(V[245])else V[245]=0 end;local bsh=0;if brake then if mph<6 then bsh=5 elseif brakeLoad>.72 and rpmDrop>5000 then bsh=3 elseif steerLoad>.22 then bsh=4 elseif V[245]<6 and brakeLoad>.35 then bsh=1 else bsh=2 end end;V[240]=bsh
...
local function draw(m,r)
 local T=lcd.drawText;local D=lcd.drawNumber;local Z=SMLSIZE;local n=m_floor;local q=r or 0;local st=(V[38]>0 and"TC")or(V[39]>0 and"ABS")or(V[620]>1 and"AIR")or(V[624]>0 and"ESC")or(V[1]~=0 and"TRUTH")or"AI";local ls=(V[709]>0 and"LEARN")or(V[543]>0 and"SAFE")or(V[119]>.55 and"UNCERT")or"READY";lcd.clear();lcd.drawFilledRectangle(0,0,128,9);T(2,1,"A15YA",Z+INVERS);T(91,1,st,Z+INVERS);T(111,1,(V[715]==1 and"L+")or(V[715]==2 and"L-")or(V[715]==3 and"RB")or"--",Z+INVERS)
...
local function init()
 setgv(0,0);setgv(1,0);setgv(3,100);setgv(4,0);setgv(7,1024);loadMem();pcall(collectgarbage,"collect");V[654]=1;V[601]=50;V[602]=50;V[607]=50;V[608]=50;V[610]=50;V[612]=50;for i=629,635 do V[i]=V[i]>0 and V[i] end;V[638]=V[638]>0 and V[638];V[132]=V[132];V[290]=12;V[293]=0;V[303]=100;V[505]=V[505]>0 and V[505];bootStage=0;bootStable=0;bootTick=getTime()
end
local function bootStep()
 ...
 if bootStage==6 then if getTime()-bootTick<50 then return false end;for i=1,36 do X[i]=0 end;X[1]=99;X[4]=-1;X[7]=1;X[10]=99;X[12]=1;X[31]=50;X[34]=50;V[292]=bootSb;V[239]=V[330];V[321]=V[321]>0 and V[321]or(V[329]);V[322]=V[322]>0 and V[322];bb_line(88,p2221(18,bootSb,95,0),0);bb_line(89,p2221(18,24,11,4),0);setgv(0,V[169]);setgv(1,V[30]);setgv(3,100);setgv(4,0);setgv(7,1024);X[7]=(bootSb==31 and gv(3)>=95 and gv(7)>=1000)and 1 or 0;warm=120;pcall(collectgarbage,"collect");bootStage=7;return false end
 return true
end
