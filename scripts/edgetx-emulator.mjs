...
const root=process.cwd(),R=path.join(root,'reports'),base=path.basename(file,'.lua'),h=path.join(R,`${base}-edgetx-emulator.lua`),out=path.join(R,`${base}-edgetx-emulator.json`);fs.mkdirSync(R,{recursive:true});
const candidate=JSON.stringify(path.resolve(file));
...
io.open=function(p,m)local mode=m or'r';if mode:find('r')and files[p]==nil then return nil end;local x=setmetatable({path=p,mode=mode,pos=1,buf=mode:find('a')and(files[p]or'')or''},H);return x end
...
local function frame(m,n)for i=1,n do now=now+3;local ok,e=call(m,'run',0);if not ok then return false,e end;if i%2==0 then ok,e=call(m,'background');if not ok then return false,e end end end;return true end
local results={};local function scenario(name,seed,steps)resetRuntime();files['/LOGS/aicmem.csv']=seed;local m,e=loadController();if not m then results[#results+1]={name=name,passed=false,error=e};return end;local ok;ok,e=call(m,'init');if ok then ok,e=steps(m)end;results[#results+1]={name=name,passed=ok,error=e,tct=gv[7],gyr=gv[1],lcd=lcdCalls,sounds=#sounds,wrote=writes['/LOGS/aicmem.csv']~=nil,writeBytes=#(writes['/LOGS/aicmem.csv']or'')}
...
scenario('car-off',nil,function(m)vals.RxBt=0;vals.RQly=0;vals.RPM=0;vals.GSpd=0;local ok,e=frame(m,20);if not ok then return false,e end;if(gv[7]or 1024)<1000 then return false,'car-off-not-fail-open' end;return true end)
...
scenario('scheduler-restart',nil,function(m)vals.RxBt=7.8;vals.RQly=100;vals.Sats=8;vals.GSpd=10;vals.RPM=14000;assert(frame(m,20));killed=true;killed=false;local n,e=loadController();if not n then return false,e end;assert(call(n,'init'));return frame(n,20)end)
