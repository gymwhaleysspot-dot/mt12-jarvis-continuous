...
const file=process.argv[2];if(!file)throw Error('usage: node scripts/edgetx-emulator.mjs <candidate.lua>');
const root=process.cwd(),R=path.join(root,'reports'),base=path.basename(file,'.lua'),profilePath=path.join(R,'edgetx-mt12-profile.json'),h=path.join(R,`${base}-edgetx-emulator.lua`),out=path.join(R,`${base}-edgetx-emulator.json`);fs.mkdirSync(R,{recursive:true});
if(!fs.existsSync(profilePath)){const s=spawnSync(process.execPath,['scripts/sync-edgetx-mt12.mjs'],{stdio:'inherit',timeout:180000});if(s.status!==0)process.exit(s.status||2)}
const profile=JSON.parse(fs.readFileSync(profilePath,'utf8'));if(profile.passed!==true||!profile.commit)throw Error('official EdgeTX MT12 profile unavailable');
const W=Number(profile.display?.width||128),HGT=Number(profile.display?.height||64),tick=Number(profile.timing?.mixerPeriodTicks||3),bg=Number(profile.timing?.backgroundPeriodTicks||6),candidate=JSON.stringify(path.resolve(file));
const lua=`local realio=io
SMLSIZE=1;MIDSIZE=2;DBLSIZE=4;INVERS=8;BLINK=16;RIGHT=32;CENTER=64;PREC1=128;PREC2=256;LCD_W=${W};LCD_H=${HGT}
...
local function loadController()local ok,m=pcall(dofile,${candidate});if not ok then return nil,'load:'..tostring(m)end;if type(m)~='table'then return nil,'module-return'end;return m end
...
local function frame(m,n)for i=1,n do now=now+${tick};local ok,e=call(m,'run',0);if not ok then return false,e end;if i%${bg}==0 then ok,e=call(m,'background');if not ok then return false,e end end end;return true end
...
`;
fs.writeFileSync(h,lua);const syntax=spawnSync('luac5.3',['-p',h],{encoding:'utf8',timeout:15000});if(syntax.status!==0){fs.writeFileSync(out,JSON.stringify({schema:2,engine:'official-edgetx-source-backed-mt12-emulator',edgetxCommit:profile.commit,candidate:base,passed:false,error:`generated-emulator-syntax: ${syntax.stderr||syntax.stdout}`},null,2));process.stderr.write(syntax.stderr||syntax.stdout||'generated emulator syntax failure');process.exit(2)}const r=spawnSync('lua5.3',[h],{encoding:'utf8',timeout:60000});if(!fs.existsSync(out))fs.writeFileSync(out,JSON.stringify({schema:2,engine:'official-edgetx-source-backed-mt12-emulator',edgetxCommit:profile.commit,candidate:base,passed:false,error:r.error?.message||r.stderr||'emulator produced no report'},null,2));process.stdout.write(r.stdout||'');process.stderr.write(r.stderr||'');process.exit(r.status??2);