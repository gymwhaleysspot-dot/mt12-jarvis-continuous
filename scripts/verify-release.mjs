...
if(candBytes.length>cfg.normalized_luac_limit)fail(`LUAC exceeds hard limit: ${candBytes.length}`);
const margin=cfg.normalized_luac_limit-candBytes.length;
...
const manifest={status:'independently-verified',name,sourceBytes:srcBytes.length,sourceSha256:sha(srcBytes),normalizedBytes:candBytes.length,normalizedSha256:sha(candBytes),deploySha256:sha(depBytes),limit:cfg.normalized_luac_limit,margin,minimumMargin:cfg.minimum_luac_margin||0,reproducible:true,verifiedAt:new Date().toISOString()};
