...
if(manifest.engine!=='MICHAEL_V51'||manifest.languages.length!==12)throw Error('Wrong polyglot manifest');
for(const token of ['#version 300 es','MICHAEL_V51','energy-aware clearcoat'])if(!fs.readFileSync('engine/shaders/michael-v48.glsl','utf8').includes(token))throw Error('GLSL contract missing '+token);
for(const token of ['@vertex','@fragment','MICHAEL_V51'])if(!fs.readFileSync('engine/shaders/michael-v48.wgsl','utf8').includes(token))throw Error('WGSL contract missing '+token);
