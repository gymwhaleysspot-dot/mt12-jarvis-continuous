...
const limit=Number(process.env.MT12_LIMIT||88944);
...
function acceptedCandidates(){
  ...
  return fs.readdirSync(accepted).filter(x=>x.endsWith('-changelog.json')).map(name=>{
    ...
    const bytes=Number(j.files?.luac?.bytes??0);
    const margin=Number(j.files?.luac?.margin??(bytes?limit-bytes:0));
    return{base,score:Number(sim?.score??j.score??0),margin,bytes};
  }).filter(Boolean);
}

const acceptedRows=acceptedCandidates().sort((a,b)=>b.score-a.score||b.margin-a.margin);
...
const evaluated=rows.map(x=>{
  ...
  const gateBytes=Number(gate?.files?.luac?.bytes||0);
  const effectiveMargin=x.margin||Number(gate?.files?.luac?.margin||(gateBytes?limit-gateBytes:0));
  const reasons=[];
...
const decision={schema:1,champion,policy:{limit,minGain,hardMargin,reserveTarget,requiredMargin},selected,candidates:evaluated,status:selected?'promote':'hold-champion',created:new Date().toISOString()};
fs.writeFileSync(path.join(reports,'champion-decision.json'),JSON.stringify(decision,null,2));
if(selected)process.stdout.write(`${selected.score}\t${selected.family}\t${selected.base}\t${selected.margin}\n`);
