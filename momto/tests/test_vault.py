...
def test_private_vault_ingests_and_deduplicates(tmp_path,monkeypatch):
    monkeypatch.chdir(tmp_path); source=Path("synthetic-record.txt"); source.write_text("Adoption placement occurred on 1980/04/12. Birth mother information was recorded.\n",encoding="utf-8")
    ...
    doc_id=ingest_document(str(source)); facts=extract_document_facts(doc_id)
    assert any(x["fact_type"]=="date" for x in facts) and any(x["fact_type"]=="relationship" for x in facts)
