...
const src=fs.readFileSync(inc,'utf8');if(/document\.createElement\(['\"]canvas['\"]\)/.test(src)||/requestAnimationFrame\s*\(/.test(src))throw Error('Production 224 must not create canvas/RAF');
if(!/arena119=function\(g\)/.test(src))throw Error('Production 224 must own the final arena119 painter');
if(/atlas117\.arena/.test(src))throw Error('Production 224 may not use the static atlas117 arena');
fs.writeFileSync(core,s);
let h=fs.readFileSync(htmlPath,'utf8'),re=/jarvis\/core-survivor\.js(?:\?v=[^"'<>\s]*)?/;if(!re.test(h))throw Error('core script missing');h=h.replace(re,'jarvis/core-survivor.js?v=20260817production224replay59');fs.writeFileSync(htmlPath,h);
console.log({production:224,bytes:s.length,sceneAuthority:'GLOBAL_ATOMIC_STAGE_OPPONENT_ARENA',finalArenaPainter:'ARENA119_STAGE_SLOT',staticArena:'RETIRED',renderer:'EXISTING_SINGLE'});
