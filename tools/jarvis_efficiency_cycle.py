...
def _postprocess()->None:
    ...
    for c in result.get('candidates',[]):
        ...
        seed_growth=max(0,size-seed_bytes)
        ...
        c['resourceEfficiency']={
            ...
            'prebuildSeedNormalizedBytes':seed_bytes,
            'prebuildBytesReclaimed':int(reclaim.get('bytesReclaimed') or 0),
            'candidateNormalizedBytes':size,
            'bytesReclaimedVsParent':saved,
            'bytesSpentVsParent':growth,
            'bytesSpentAfterReclaim':seed_growth,
        ...
        if manifest_path.is_file():
            ...
            m['resourceEfficiency']=c['resourceEfficiency'];m['preEfficiencyScore']=c['preEfficiencyScore'];m['score']=c['score'];m['promotionEfficiencyEligible']=c['promotionEfficiencyEligible']
    ...
    result['resourcePolicy']={
        'schema':'JARVIS-RESOURCE-EVOLUTION-2','cycle':['reclaim','rethink','build','compete','synthesize','compress','verify','inherit'],
        'parentNormalizedBytes':parent_bytes,'prebuildSeedNormalizedBytes':seed_bytes,'prebuildBytesReclaimed':int(reclaim.get('bytesReclaimed') or 0),
        'prebuildChanges':reclaim.get('changes',[]),'hardCeilingBytes':MAX_BYTES,'minimumIntelligencePer100GrowthBytes':MIN_INTEL_PER_100_GROWTH_BYTES,
        'rule':'PREBUILD_RECLAMATION_AND_SIZE_EFFICIENCY_ALWAYS_ON','growthWithoutEnoughIntelligence':'NOT_PROMOTION_ELIGIBLE',
    }
