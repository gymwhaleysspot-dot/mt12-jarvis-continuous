...
def enable_v40():
    ...
    if graphics_changed or topology_changed:
        old=active_version(synced);new=old+1;synced=synced.replace(f'michael{old}',f'michael{new}');RUNTIME.write_text(synced);HTML.write_text(HTML.read_text().replace(f'michael{old}',f'michael{new}'));print(json.dumps({'changed':True,'graphics':'v40','topology':'OWNER_V42','old':old,'new':new}));return
    print(json.dumps({'changed':False,'graphics':'v40','topology':'OWNER_V42'}))
