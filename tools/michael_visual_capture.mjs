...
const expectedVersion=process.env.MICHAEL_EXPECT_VERSION||'MICHAEL_V66';
const expectedIdentity=process.env.MICHAEL_EXPECT_IDENTITY||'MJX7303_C3_WRC_V66';
...
try{
  log(`launch chromium size=${size} views=${views.join(',')} expect=${expectedVersion}`);
  ...
  await page.waitForFunction(({version,identity})=>{
    const g=window.__MICHAEL_GRAPHICS||{};
    return g.version===version&&g.topologySanity===true&&g.driverPresent===true&&g.navigatorPresent===true&&Number(g.occupantCount)===2&&(!identity||g.c3Identity?.identity===identity||g.c3Identity===identity);
  },{version:expectedVersion,identity:expectedIdentity},{timeout:readyTimeout});
  ...
  const metrics=await page.evaluate(()=>{const g=window.__MICHAEL_GRAPHICS||{},c=document.querySelector('#raceCanvas'),gl=c?.getContext('webgl2')||c?.getContext('webgl');return{graphics:g,ready:window.__MICHAEL_READY===true,model:window.__MICHAEL_MODEL===true,error:window.__MICHAEL_ERROR||null,frameMs:Number(g?.physicalFrameMs||g?.frameMs||16.7),drawables:Number(g?.drawables||g?.drawCalls||0),quality:Number(g?.quality||0),version:g?.version||null,topologySanity:g?.topologySanity===true,malformedTopologyCount:Number(g?.malformedTopologyCount||0),driverPresent:g?.driverPresent===true,navigatorPresent:g?.navigatorPresent===true,occupantCount:Number(g?.occupantCount||0),c3Identity:g?.c3Identity||null,canvas:{width:c?.width||0,height:c?.height||0,clientWidth:c?.clientWidth||0,clientHeight:c?.clientHeight||0},webgl:!!gl}});
  ...
  if(expectedIdentity&&id!==expectedIdentity)throw new Error(`Michael identity contract failed: expected ${expectedIdentity}, got ${id}`);
  if(!metrics.driverPresent||!metrics.navigatorPresent||metrics.occupantCount!==2)throw new Error(`Michael cockpit incomplete: driver=${metrics.driverPresent} navigator=${metrics.navigatorPresent} occupants=${metrics.occupantCount}`);
