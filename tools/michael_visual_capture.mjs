...
const shotTimeout=Math.max(3000,Number(process.env.MICHAEL_SCREENSHOT_TIMEOUT_MS||12000));
...
try{
  ...
  // Wait for two actual animation frames so the first owner-truth capture is a rendered frame, not just DOM-ready.
  await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
  for(const view of views){
    log(`view ${view}`);
    await page.locator(`[data-view="${view}"]`).click({timeout:shotTimeout});
    await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
    const box=await page.locator('#raceCanvas').boundingBox();
    if(!box||box.width<16||box.height<16)throw new Error(`raceCanvas has invalid capture bounds for ${view}: ${JSON.stringify(box)}`);
    // Page clip does not require Playwright's element-stability gate, which is incompatible with a continuously rendered WebGL canvas.
    await page.screenshot({path:path.join(out,`${view}.png`),clip:{x:Math.max(0,box.x),y:Math.max(0,box.y),width:Math.min(box.width,size-Math.max(0,box.x)),height:Math.min(box.height,size-Math.max(0,box.y))},timeout:shotTimeout,animations:'disabled'});
  }
  const metrics=await page.evaluate(()=>{const g=window.__MICHAEL_GRAPHICS||null,c=document.querySelector('#raceCanvas'),gl=c?.getContext('webgl2')||c?.getContext('webgl');return{
    ...
    ready:window.__MICHAEL_READY===true,v39:window.__V39_READY===true,model:window.__MICHAEL_MODEL,error:window.__MICHAEL_ERROR||null,
  ...
  // Local favicon/optional requests are reported, but missing core JS/GLB/WASM assets are fatal.
