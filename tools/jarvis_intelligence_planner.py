...
def main()->int:
 ...
 synthesis={'slot':6,'area':'synthesis-all-five','name':'Synthesize all five hypotheses','inputs':[h['area'] for h in hypotheses],'rule':'compose only compatible, genuinely new mutations on the active evolution parent; external research is advisory evidence only; reject on canonical-floor regression, compile failure, duplicate bytecode, or MT12 size violation','measurement':'beat each constituent on aggregate benefit without new regressions'}
 experiment={'id':eid,'area':hypotheses[0]['area'],'name':'Research-aware evolutionary five-hypothesis generation plus synthesis','parent':champion,'canonicalFloor':canonical_floor,'hypothesis':'Run five independent hypotheses on the active evolution parent, then build a sixth synthesis while continuously checking the immutable canonical floor.','mutations':[m for h in hypotheses for m in h['mutations']],'measurement':'maximize information gained per generation while preserving defended behavior','expectedInformationGain':round(sum(h['expectedInformationGain'] for h in hypotheses)/5,4),'portfolio':hypotheses,'synthesis':synthesis,'externalResearch':{'runId':research.get('researchRun'),'topic':research.get('topic'),'acceptedBridgeFindings':research.get('acceptedBridgeFindings',0),'policy':research.get('policy',{})},'acceptanceCriteria':{'byteNovel':True,'behaviorallyNovel':True,'noRegression':True,'minimumConfidence':.85,'mustCompareAgainstChampion':True,'mustPreserveCanonicalFloor':True,'inheritedCapabilityBonus':False,'fiveIndependentHypotheses':True,'sixthSynthesisRequired':True,'externalResearchMayDirectlyMutateController':False},'requiredTests':['Lua 5.3 compile','MT12 normalization','static safety audit','historical novelty audit','canonical defended-lineage gate','runtime memory/size check','bench and road evidence before canonical promotion'],'reason':'Portfolio evolves cumulatively from the best static-validated synthesis while retaining a separate immutable safe canonical fallback.','createdAt':now()}
 doc={'schema':'JARVIS-EXPERIMENT-PLAN-4','generatedAt':now(),'champion':champion,'evolutionParent':champion,'canonicalFloor':canonical_floor,'nextExperiment':experiment,'portfolio':hypotheses,'synthesis':synthesis,'rankedUnknowns':ranked,'externalResearchState':experiment['externalResearch'],'graphState':{'nodes':graph.get('nodeCount',0),'relationships':graph.get('relationshipCount',0)},'buildState':{'unique':builds.get('uniqueBuilds',0),'duplicatesRejected':builds.get('duplicatesRejected',0)}}
 save(OUT,doc);save(PLAN,doc);hist['schema']=4;hist.setdefault('selections',[]).append({'at':now(),'areas':[h['area'] for h in hypotheses],'experiment':eid,'champion':champion,'canonicalFloor':canonical_floor,'strategy':'research-aware-evolutionary-five-plus-synthesis','researchRun':research.get('researchRun')});hist['selections']=hist['selections'][-500:];hist['updatedAt']=now();save(HISTORY,hist)
