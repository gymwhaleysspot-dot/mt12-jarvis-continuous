...
function propose(renderDir,out){
  ...
  const base=runInternal(['propose',renderDir,sensor],sensor),mem=readMemory();let neural=null;try{neural=runNeural(['predict',renderDir,neuralOut],neuralOut)}catch{}
  ...
  const result={...base,candidateParameters:cand,diagnosis:notes,nickelle:{name:identity.name,version:identity.version,primary:true,weakest:ranked.slice(0,5),learnedDirections:Object.keys(mem.parameterDirections).length,cycles:mem.cycles,neuralVision:neural?{scores:neural.scores,confidence:neural.confidence,samples:neural.samples}:null},rationale:[...(base.rationale||[]),'Nickelle selected/damped mutations using persistent win-loss memory and subsystem priorities.','Neural Vision observes raw five-view pixels and gains decision authority only as training confidence increases.']};
...
function compare(baseDir,candDir,proposalPath,out){
  ...
  const iv=runInternal(['compare',baseDir,candDir,sensor],sensor),mem=readMemory(),proposal=JSON.parse(fs.readFileSync(proposalPath,'utf8'));let nv=null;try{nv=runNeural(['compare',baseDir,candDir,neuralOut],neuralOut)}catch{}
  ...
  const result={...iv,promote,regressions:reg,reason:`Nickelle fused verdict: internal gain ${ivGain.toFixed(3)}, neural gain ${neuralGain.toFixed(3)}, neural authority ${(authority*100).toFixed(1)}%, fused gain ${blendedGain.toFixed(3)}, threshold ${threshold.toFixed(2)}.`,nickelle:{name:identity.name,primary:true,proposalParameters:proposal.candidateParameters||{},memoryCycles:mem.cycles,neuralVision:nv,neuralAuthority:authority,blendedGain}};
  fs.writeFileSync(out,JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));
}
