...
def _protected(text:str)->list[str]:
    ...
    return hotfix.protected_checks(text)
...
def _dead_stage_local_proposal(text:str):
    ...
    # Safe only when s has no reference between this declaration and the end of jAS.
...
def _unused_simple_local_proposals(text:str):
    # Only simple single-name locals whose RHS is side-effect-free. A local referenced once
    # in the entire source is definition-only and therefore behaviorally dead.
...
def _unused_local_function_proposals(text:str):
    # Conservative one-line function form only. No multiline parser guesses.
    pat=re.compile(r'(?m)^local function ([A-Za-z_]\w*)\([^\n]*?\).*?end\s*$')
    protected={'zBrain','memOpt','watch','bad','lerp','fid','val','gv','setgv','rdmem','wrmem'}
    for m in list(pat.finditer(text)):
        name=m.group(1)
...
def reclaim()->dict:
    ...
    current=before_text;current_size=before_size;changes=[];rounds=0

    # Fixed-point discovery: every accepted reduction changes the search surface, so scan again.
    while rounds<8:
    ...
    doc={'schema':'JARVIS-PREBUILD-RECLAIM-2','release':release,'source':str(src.relative_to(ROOT)),'seed':str(SEED.relative_to(ROOT)),'parentNormalizedBytes':before_size,'reclaimedNormalizedBytes':current_size,'bytesReclaimed':max(0,before_size-current_size),'discoveryRounds':rounds,'acceptedChanges':len(changes),'changes':changes,'protectedContractErrors':errors,'policy':'DISCOVER+TRIAL_COMPILE; ONLY_BEHAVIOR_EQUIVALENT_PROVABLE_RECLAMATION; NEVER_REMOVE_LIVE_FEATURES','acceptanceRule':'EACH_CHANGE_MUST_LOAD_LUA53+PASS_PROTECTED_CONTRACT+NORMALIZE_MT12+STRICTLY_REDUCE_BYTES'}
