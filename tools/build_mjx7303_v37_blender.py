...
def material(name,color,metal=0,rough=.3,coat=.4,alpha=1):
    ...
    if alpha<1:pinput(b,['Alpha'],alpha);m.blend_method='BLEND';m.use_screen_refraction=True
    return m
...
S.world.color=(.018,.022,.028);S.render.engine='BLENDER_EEVEE';S.render.resolution_x=1200;S.render.resolution_y=900;S.render.resolution_percentage=100;S.render.image_settings.file_format='PNG'
