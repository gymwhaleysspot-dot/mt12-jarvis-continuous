...
for(const marker of ['ONE_CANONICAL_STAGE_MANIFEST','ATOMIC_OPPONENT_ACTOR_TARGET_ARENA_BIND','LEGACY_SCENE_AUTHORITY_RETIREMENT','EVENT_IDENTITY_NORMALIZATION','STALE_BOSS_PRUNING','REPLAY_RUNTIME_PROOF'])if(!built.includes(marker))throw Error('Production 234 marker missing '+marker);
for(const forbidden of ['core-survivor-221.js','core-survivor-230.js'])if(built.includes(forbidden))throw Error('stale hard-cache reference leaked into canonical runtime: '+forbidden);
console.log({production:234,runtimeProduction:234,artifact:out,result:'PASS'});
