...
let html=fs.readFileSync(htmlPath,'utf8');const script=/jarvis\/core-survivor\.js(?:\?v=[^"'<>\s]*)?/;if(!script.test(html))throw Error('survivor core script tag not found');html=html.replace(script,'jarvis/core-survivor.js?v=20260816production200');fs.writeFileSync(htmlPath,html);
console.log({built:layers.map(x=>x.marker),bytes:source.length,cache:'production200'});
