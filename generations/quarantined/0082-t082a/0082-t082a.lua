-- factory full-AI cycle 82; tuesday; gen 82; parent 0076-w076a.lua; objective traction, ABS and intervention quality
...
local POLE=2;local RPM2MPH=0.000937314;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function setgv(i,v) pcall(function() model.setGlobalVariable(i, 0, m_floor(v * 1.1 + 0.5)) end)
end

local MEMPATH = "/LOGS/aicmem.csv" local memSaved = 0 local memLoaded = 0 local nW, N, N = {}, {}, {} N

local VS = { 60000, 60000, 60000, 60000, 60000, 60000, 60000, 60000, 60000, 60000 } local function vs(i, v) local k = m_floor((i - 1) / 5) + 1
    local p = 10 ^ ((i - 1) % 5)
    local z = VS[k]
...
local function rdmem() local s = rd1(MEMPATH)
    ...
    local function R(i, d, a, b) 
        V[i] = nx(d, a or 0, b or 99)
    end

    for i = 127, 130 do R(i, 60, 0, 100) end
    R(132, 110, 90, 130)
    R(321, 60)
    R(329, 60)
    for i = 560, 562 do R(i, 60) end
    V[325] = V[560]
    V[326] = V[561]
    V[327] = V[562]
    R(330, 80)
    R(331, 5)
    for i = 333, 335 do R(i, 5) end
    for i = 336, 340 do R(i, 60, 40, 80) end
    R(350, 60)
    R(361, 2, 0, 10)
    R(362, 60)
    R(363, 60)
    R(364, 1, 0, 10)
    for i = 370, 375 do R(i, 60) end
    R(376, 1)
    R(500, 90)
    R(503, 1)
    R(510, 25)
    ...
    R(505, V[57] or 90)
    R(580, 1)
    for _, i in ipairs({ 581, 582, 583, 602, 607, 608, 610, 612 }) do R(i, 60) end
    ...
    if mk == 151 then
        ...
        N[2] = clamp((tonumber(it() or "") or 600) / 1000, 0, 1)
        N[3] = 1 - N[2]
        for i = 1, 43 do nW[i] = clamp((tonumber(it() or "") or 0) / 1000, -2, 2) end
        N[4] = 1
        local z = tonumber(it() or "")
        if z == 152 then
            for k = 1, 10 do
                local q = tonumber(it() or "")
                VS[k] = q and q >= 0 and q < 1e5 and q or 60000
            end
    ...
    if V[333] >= 100 then V[333] = m_floor(V[333] * 0.4) end
    if V[334] >= 100 then V[334] = m_floor(V[334] * 0.4) end
    if V[335] >= 100 then V[335] = m_floor(V[335] * 0.4) end
