-- factory isolated-AI cycle 102; tuesday; gen 102; parent 0074-t074a.lua; objective traction, ABS and intervention quality
...
local POLE=2;local RPM2MPH=0.000928876;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function trait(old, score, rate) old = clamp(old or 50, 0, 99)
    ...
    return clamp(old + (score - old) * (rate or 0.02) * clamp(0.75 + (V[638] or 50) / 100, 0.85, 1.25), 0, 99)
end

local function md(j, x, b) local d = x - (V[b] or x)
    local s = (V[j] - 50) * d > 0 and 1 or 0
    V[j] = trait(V[j], clamp(50 + d * 4, 0, 99), 0.5)
    V[b] = clamp((V[b] or x) * 0.5 + x * 0.5, 0, 99)
    return s
end
