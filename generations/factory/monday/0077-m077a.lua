-- factory full-AI cycle 77; monday; gen 77; parent 0076-w076a.lua; objective truth prediction, calibration and sensor resilience
...
local POLE=2;local RPM2MPH=0.000930154;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function setgv(i,v) pcall(function() model.setGlobalVariable(i, 0, m_floor(v + 0.5)) end)
end

local MEMPATH = "/LOGS/aicmem.csv" local memSaved = 0 local memLoaded = 0 local nW, N , N
local VS = { 55555, 55555, 55555, 55555, 55555, 55555, 55555, 55555, 55555, 55555 } local function vs(i, v) local k = m_floor((i - 1) / 5) + 1
    local p = 10 ^ ((i - 1) % 5)
    local z = VS[k]
    local o = m_floor(z / p) % 10
    if v then VS[k] = z + (v - o) * p end
    return o
end

local function rd1(p) local f = io.open(p, "r")
    if not f then return end
    local s = io.read(f, 512)
    pcall(io.close, f)
    return s and string.match(s, "([^\r\n]+)")
end

local function rdmem() local s = rd1(MEMPATH)
    if not s then return end
    local it = string.gmatch(s, "[^,]+")
    local tag = it()
    if not string.match(tag or "", "^AIC%d+[A-Z]$") then return end
    local n = 0
    local function nx(d, a, b) local x = it()
        n = n + 1
        return clamp(tonumber(x) or d, a, b)
    end
    nx(0, 0, 99)
    nx(0, 0, 99)

    local function R(i, d, a, b) V[i] = nx(d, a or 0, b or 99)
    end

    for i = 127, 130 do R(i, 50, 0, 100) end
    R(132, 100, 85, 115)
    R(321, 50)
    R(329, 50)
    for i = 560, 562 do R(i, 50) end
    V[325] = V[560]
    V[326] = V[561]
    V[327] = V[562]
    R(330, 70)
    R(331, 0)
    for i = 333, 335 do R(i, 0) end
    for i = 336, 340 do R(i, 50, 35, 65) end
    R(350, 50)
    R(361, 1, 0, 9)
    R(362, 50)
    R(363, 50)
    R(364, 0, 0, 9)
    for i = 370, 375 do R(i, 50) end
    R(376, 0)
    R(500, 85)
    R(503, 0)
    R(510, 20)
    ...
    R(505, V[57] or 85)
    R(580, 0)
    for _, i in ipairs({ 581, 582, 583, 602, 607, 608, 610, 612 }) do R(i, 50) end
    u3(nx(505050, 0, 9999999), 629)
    u3(nx(505050, 0, 9999999), 632)
    u3(nx(500050, 0, 9999999), 635)

    local mk = tonumber(it() or "")
    if mk == 151 then
        N[1] = clamp(tonumber(it() or "") or 0, 0, 9999)
        N[2] = clamp((tonumber(it() or "") or 500) / 1000, 0, 1)
        N[3] = 1 - N[2]
        for i = 1, 43 do nW[i] = clamp((tonumber(it() or "") or 0) / 1000, -1.5, 1.5) end
        N[4] = 1
        local z = tonumber(it() or "")
        if z == 152 then
            for k = 1, 10 do
                local q = tonumber(it() or "")
                VS[k] = q and q >= 0 and q < 1e5 and q or 55555
            end
            local z = tonumber(it() or "") or 505050
            V[672] = m_floor(z / 10000)
            V[673] = m_floor(z / 100) % 100
            V[678] = z % 100
            local a = tonumber(it() or "")
            local b = tonumber(it() or "")
            if a then
                V[671] = m_floor(a / 10000)
                V[674] = m_floor(a / 100) % 100
                V[675] = a % 100
            end
            if b then
                V[676] = m_floor(b / 10000)
                V[677] = m_floor(b / 100) % 100
            end
        end
    end

    if V[333] >= 90 then V[333] = m_floor(V[333] * 0.5) end
    if V[334] >= 90 then V[334] = m_floor(V[334] * 0.5) end
    if V[335] >= 90 then V[335] = m_floor(V[335] * 0.5) end
...
local function i9(x) return clamp(x + 1, 0, 99)
end

local function trait(old, score, rate) old = clamp(old or 50, 0, 99)
    score = clamp(score or 50, 0, 99)
    return clamp(old + (score - old) * (rate or 0.03) * clamp(0.80 + (V[638] or 50) / 100, 0.90, 1.20), 0, 99)
end

local function md(j, x, b) local d = x - (V[b] or x)
    local s = (V[j] - 50) * d > 0 and 1 or 0
    V[j] = trait(V[j], clamp(50 + d * 5, 0, 99), 0.6)
    V[b] = clamp((V[b] or x) * 0.6 + x * 0.4, 0, 99)
    return s
end
local function traitLearn() local clean=clamp((V[333])/(m_max(1,V[289]or 1))*100,0,99)
 local ev=clamp(V[289],0,99)
 local sev=clamp(V[302],0,99)
 local swing=clamp((V[341])+(V[342])*1.2+(V[343]),0,99)
 local data=clamp(V[347]or 80,0,99)
 local mismatch=clamp(V[304]or V[216],0,99)
 local speed=clamp(V[331],0,60)
 local commit=clamp(40+speed*.8+clean*.25+(V[325]or 50)*.15-sev*.5-ev*.05,0,99)
 local compose=clamp(50+clean*.5+data*.1-sev*.5-swing*.4,0,99)
 local adapt=clamp(50+((V[239]or 65)-65)*.6+(data-80)*.25-mismatch*.2-(V[119])*15,0,99)
 local sty=clamp((V[560]or V[325]or 50)*.45+(V[561]or V[326]or 50)*.35+(V[562]or V[327]or 50)*.2,0,99)
 local note=clamp(45+data*.4-mismatch*.25+((V[239]or 65)-65)*.3+clean*.15-sev*.1,0,99)
 V[370]=trait(V[370],commit,.015)
 V[371]=trait(V[371],compose,.015)
 V[372]=trait(V[372],adapt,.015)
 V[373]=trait(V[373],sty,.018)
 V[374]=trait(V[374],note,.015)
 V[375]=clamp((V[370]+V[371]+V[372]+V[374])*.3,0,99)
 V[376]=i9(V[376])
end

local function wrmem() if memSaved>0 then return end
 memSaved=1
 local a={m_max(V[513],100-(V[239]or 50),100-(V[602]or 50)),m_max(V[520],V[606]),m_max(V[503],V[262])-(V[501])*.25,100-(V[168])+(V[69])*25}
 local k=1
 local h=a[1]
 local l=0
 for i=2,4 do 
 ...
 local cc=clamp(h-l,0,99)
 if cc<10 or k==4 then 
  V[638]=m_max(20,V[638]-5)
 elseif k~=3 then 
  V[503]=V[503]*.9 
 end
 V[405]=p2221(k,cc,V[520],(V[503])/10)
 if V[655]>0 and (V[541]or 50)>40 and (V[543])==0 then 
  traitLearn()
  V[548]=0 
 else 
  V[548]=1 
 end
 local f=io.open(MEMPATH,"w")
 if not f then return end
 local clean=((V[302])<25 and (V[303])>75)and 1 or 0
 local tcT=clamp((V[334])+(V[100]),0,99)
 local abT=clamp((V[335])+(V[101]),0,99)
 local cr=clamp((V[333])/m_max(1,V[289]or 1)*100,0,99)
 local n=md(629,cr,580)+md(630,V[521]or 50,581)+md(631,V[530]or 50,582)+md(632,100-(V[543])*40,583)
 V[633]=clamp(V[633]+(n>2 and 10 or -5),0,99)
 local x=((V[239]or 50)>70 and cr<50)or((V[610]or 50)>70 and(V[606])>45)or((V[500]or 50)>80 and V[628]>60)
 V[634]=trait(V[634],x and 95 or 25,.3)
 V[635]=trait(V[635],100-m_abs((V[239]or 50)-cr),.3)
 V[638]=clamp(50+(V[635]-50)*.5-V[634]*.4+(V[633]-50)*.2,20,85)
 if V[634]>70 then V[603]=m_max(V[603],25) end
 if V[333]>=90 then V[333]=m_floor(V[333]*.6) end
 if V[334]>=90 then V[334]=m_floor(V[334]*.6) end
 if V[335]>=90 then V[335]=m_floor(V[335]*.6) end
 tcT=clamp((V[334])+(V[100]),0,99)
 abT=clamp((V[335])+(V[101]),0,99)
 local row={"AIC9Z",18,24}
 local function A(x,a,b) row[#row+1]=q(x,a,b) end
 for i=127,130 do A(V[i],0,100) end
 A(V[132],85,115)
 A(V[321])
 A(V[329])
 A(V[560]or V[325])
 ...
 A(tcT)
 A(abT)
 for i=336,340 do A(V[i],35,65) end
 A(V[350])
 A(V[361]or 1,0,9)
 A(V[362]or 50)
 A(V[363]or 50)
 A(V[364],0,9)
 for i=370,376 do A(V[i]) end
 for _,i in ipairs({500,503,510,511,520,521,530,533}) do A(V[i]) end
 A(V[542]or 70)
 A(V[545]or 50)
 A(V[550])
 A(V[551]or 50)
 A(V[505]or V[57]or 85)
 for _,i in ipairs({580,581,582,583,602,607,608,610,612}) do A(V[i]or 50) end
 row[#row+1]=p332(V[629],V[630],V[631])
 row[#row+1]=p332(V[632],V[633],V[634])
 row[#row+1]=p332(V[635],V[637],V[638])
 row[#row+1]=151
 row[#row+1]=m_floor(N[1]+.5)
 row[#row+1]=m_floor(N[2]*1000+.5)
 for i=1,43 do row[#row+1]=m_floor(clamp(nW[i],-1.5,1.5)*1000) end
 row[#row+1]=152
 for k=1,10 do row[#row+1]=VS[k] end
 row[#row+1]=p332(V[672],V[673],V[678])
 row[#row+1]=p332(V[671],V[674],V[675])
 row[#row+1]=p332(V[676],V[677],0)
 local out=tostring(row[1])
 for i=2,#row do out=out..","..tostring(row[i]) end
 io.write(f,out,"\n")
 pcall(io.close,f)
 V[294]=1
end
...
local sR=0;local cL=0;local srt=55;local clt=30;local sndCd=0;local sndPr=0;local sndLast=0;local metaLast=0;local vcCd=0;local vcLast=0;local vcCnt=0;local vcPr=0;local vBoot=1;local vWait=99;local vLoad=0;local vQuiet=50;local voiceBlock=0;local sndBlock=0
local function memLearn(moving) if not moving then return end
    local gpsBad = 1 - V[114]
    local tcT = clamp(60 + V[116] * 25 - (V[113] * 20) - (V[36] > 30 and 15 or 0), 0, 100)
    local rgT = clamp(60 + (V[113] + V[119] + gpsBad) * 15, 0, 100)
    local gpT = clamp(V[40] * 90, 0, 100)
    local abT = clamp(60 + V[117] * 25 - (V[37] > 30 and 12 or 0), 0, 100)
    
    V[127] = V[127] + (tcT - V[127]) * 0.015
    V[128] = V[128] + (rgT - V[128]) * 0.015
    V[129] = V[129] + (gpT - V[129]) * 0.012
    V[130] = V[130] + (abT - V[130]) * 0.012
    
    if sR > 0.4 then srt = clamp(srt + (((V[16] > 0) and 45 or 55) - srt) * 0.007, 40, 65) end
    clt = clamp(clt + (((cL > 0.5) and 20 or 30) - clt) * 0.004, 15, 45)
    
    V[131] = (V[131] + 1) % 100
end

local function bar(x, y, w, h, p) p = clamp(p or 0, 0, 1)
    lcd.drawRectangle(x, y, w, h)
    local fw = m_floor((w - 2) * p)
    if fw > 0 then lcd.drawFilledRectangle(x + 1, y + 1, fw, h - 2) end
end
...
local function vi(n) return string.find(VC, string.char(n), 1, true)
end
...
local function voice(n,p,cd)
 ...
 if p<5 and vLoad>74 then voiceBlock=i9(voiceBlock);return end
 if p<4 and (V[550])>68 then voiceBlock=i9(voiceBlock);return end
...
local function ol(p,o,v,n) local k=670+p
 V[k]=clamp((V[k]>0 and V[k] or 50)+(o>0 and 1 or -1),20,80)
 V[680]=p
 V[681]=o
 V[682]=clamp(v or 0,0,99)
 V[683]=n or 0
 if n and n>0 then vl(n,o>0 and 1 or -1) end
end

local function nt(t1,t2,t3) local o1,o2,o3=nf(N[6],N[7],N[8],N[9],N[10],N[11])
 local e1=t1-o1
 local e2=t2-o2
 local e3=t3-o3
 local lr=.0005+.002*(1-N[3])
 N[17]=e1*((o1>0 and o1<1) and .3 or 0)
 N[18]=e2*((o2>0 and o2<1) and .3 or 0)
 N[19]=e3*((o3>0 and o3<1) and .3 or 0)
 for j=1,4 do
  local h=nH[j]
  local de=0
  for o=1,3 do
   ...
   local x=N[16+o]
   de=de+x*w
   nW[k]=clamp(w+lr*x*h,-1.5,1.5)
  end
  de=de*((h>0 and h<1) and .3 or 0)
  local q=(j-1)*6+1
  for i=0,5 do
   nW[q+i]=clamp(nW[q+i]+lr*de*N[6+i],-1.5,1.5)
  end
  nW[24+j]=clamp(nW[24+j]+lr*de,-1.5,1.5)
 end
 for o=1,3 do
  nW[40+o]=clamp(nW[40+o]+lr*N[16+o],-1.5,1.5)
 end
 local er=(m_abs(e1)+m_abs(e2)+m_abs(e3))/3
 N[2]=N[2]*.98+er*.02
 N[3]=clamp(1-N[2],0,1)
 N[1]=m_min(N[1]+1,9999)
 V[277]=er*100
end

local function neural(a,b,c,d,e,f,learn) local o1,o2,o3=nf(a,b,c,d,e,f)
 if N[5]>0 then
  local tc=(V[38]>0) and clamp(.6+V[36]/100,.6,1) or 0
  local ab=(V[39]>0) and clamp(.6+V[37]/100,.6,1) or 0
  N[12]=m_max(N[12],a,d*.8,e*.5,tc)
  N[13]=m_max(N[13],b,d*.6,e*.4,ab)
  N[14]=m_max(N[14],c,d*.5,f,(V[1]~=0) and .75 or 0)
  N[5]=N[5]-1
  if N[5]==0 then
   if (N[15] or 0)>0 then
    local ok=(N[16]==1 and N[12]>.45) or (N[16]==2 and N[13]>.45) or (N[16]==3 and N[14]>.45)
    vl(N[15],ok and 1 or -1)
    V[574]=N[15]
    V[575]=N[16]
    V[577]=ok and 1 or 0
    if not ok then V[662]=110; vl(110,1) end
    N[15]=0
    N[16]=0
   end
   nt(clamp(N[12],0,1),clamp(N[13],0,1),clamp(1-N[14],0,1))
   o1,o2,o3=nf(a,b,c,d,e,f)
  end
 elseif learn>0 and getTime()%10==0 then
  N[6],N[7],N[8],N[9],N[10],N[11]=a,b,c,d,e,f
  N[12],N[13],N[14]=a,b,m_max(c,f)
  N[5]=10
 end
 V[270]=o1*100
 V[271]=o2*100
...
local function aiGate(moving,mph,rM,rough,hz,brake) local md = (mph > 3 and rM > 1) and clamp(m_abs(rM - mph) / (mph + 3), 0, 1) or 0
    local g = clamp(V[114] * 0.5 + gQ * 0.25 + (1 - V[119]) * 0.15 + (1 - md) * 0.1, 0, 1)
    if moving and mph > 3 and V[114] > 0.5 then 
        V[224] = clamp(V[224] + 1, 0, 900) 
    else 
        V[224] = clamp(V[224] - 2, 0, 900) 
    end
    if not moving or V[23] > 0 or mph < 3 then g = g * 0.4 end
    if V[161] > 0 or V[164] > 0 or rough > 0 then g = g * 0.75 end
    if brake then g = g * 0.85 end
    if hz > 0.7 then g = g * 0.8 end
    if hz > 0.7 or V[36] > 30 or V[37] > 30 then aiCool = 20 
    elseif aiCool > 0 then aiCool = m_max(0, aiCool - (0.9 + ((V[371] or 50) - 50) / 100)) end
    local gr = 0
    if not moving or mph < 3 then gr = 1 
    elseif g <= 0.6 then gr = 2 
    elseif V[114] <= 0.65 then gr = 3 
    elseif V[119] >= 0.5 then gr = 4 
    elseif V[224] <= 50 then gr = 5 
    elseif aiCool > 0 then gr = 6 
    elseif V[23] > 0 then gr = 7 
    elseif V[161] > 0 or V[164] > 0 then gr = 8 
    end
    local l = (gr == 0) and 1 or 0
    V[288] = gr
    V[215] = g * 100
    V[216] = md * 100
    V[217] = l * 100
...
local function learn(moving, slipRatio, spin, lock, rough, gtrust) local t = gtrust or 0
    local gpsBad = clamp((0.7 - t) / 0.7, 0, 1)
    local r = clamp(m_max(rough or 0, gpsBad * 0.7), 0, 1)
    local sp = clamp(m_max(slipRatio or 0, (spin or 0) * 0.8), 0, 1)
    local real = (t > 0.65 and r < 0.5) and sp or 0
    local fspin = m_max(r, gpsBad) * sp
    local good = (moving and t > 0.75 and sp < 0.2 and r < 0.3 and (lock or 0) < 0.25) and 1 or 0
    local tc = V[38] > 0 and 1 or 0
    local ab = V[39] > 0 and 1 or 0
    local tcBig = V[36] > 25 and 1 or 0
    local abBig = V[37] > 25 and 1 or 0
    local tcGood = tc * clamp(0.6 - sp, 0, 0.6) / 0.6
    local tcWeak = tc * clamp(sp - 0.5, 0, 0.6) / 0.6
    local tcHarsh = tc * tcBig * clamp(0.35 - sp, 0, 0.35) / 0.35
    local abGood = ab * (lock or 0) * clamp(0.9 - r, 0, 0.9) / 0.9
    local abHarsh = ab * abBig * clamp(0.4 - (lock or 0), 0, 0.4) / 0.4
    local u = 0.12
    local d = 0.03
    lrn(1, 1 + good * 0.04 + r * 0.03 - tcHarsh * 0.05, u, d, 0.9, 1.1)
    lrn(2, 1 + real * 0.15 + good * 0.06 - fspin * 0.12, u, d, 0.85, 1.3)
    lrn(3, 1 + abGood * 0.1 + r * 0.05 + tcWeak * 0.06 - tcHarsh * 0.09, u, d, 0.8, 1.25)
    lrn(4, 1 + fspin * 0.35 + gpsBad * 0.25 - real * 0.12 - good * 0.05, u, d, 0.75, 1.5)
    lrn(5, 1 + fspin * 0.45 + tcHarsh * 0.3 - real * 0.25 - tcWeak * 0.2, u, d, 0.7, 1.7)
    lrn(6, 1 + tcWeak * 0.25 - tcHarsh * 0.4 - fspin * 0.35, u, d, 0.55, 1.3)
    lrn(7, 1 + fspin * 0.4 + gpsBad * 0.25 + abHarsh * 0.3 - abGood * 0.2, u, d, 0.75, 1.7)
    lrn(8, 1 + real * 0.12 + tcWeak * 0.15 - fspin * 0.25 - tcHarsh * 0.2, u, d, 0.72, 1.3)
    lrn(9, 1 + good * 0.12 + tcGood * 0.1 - r * 0.12 - tcWeak * 0.1, u, d, 0.8, 1.25)
    if not moving then for i = 1, 9 do lrn(i, 1, 0.02, 0.02, 0.5, 1.8) end end
end

local function gripSM(moving, slip, tcActive, fakeScore, realScore) if not moving then 
        V[135] = 0 
        V[136] = 0 
        V[137] = 0 
        V[138] = 0 
    ...
    local st = V[135]
    if tcActive > 0 or (slip > 2.5 and (realScore or 0) > 0.3) then
        if st == 0 then 
            V[139] = slip 
            V[136] = 0 
        end
        V[135] = 1 
        V[136] = V[136] + 1
        if slip < 1.5 then V[135] = 2; V[137] = 0 end
    elseif st == 1 then
        V[135] = 2 
        V[137] = 0
    elseif st == 2 then
        ...
        if slip < 1.2 then
            local q
            if V[137] <= 5 then q = 0.1 
            elseif V[137] <= 15 then q = 0.05 
            elseif V[137] <= 30 then q = -0.02 
            else q = -0.1 end
            if (fakeScore or 0) > 0.5 then q = q * 0.3 end
            V[140] = q 
            V[40] = clamp(V[40] + q, 0.3, 1) 
            V[135] = 3 
            V[138] = 25
        elseif V[137] > 30 then
            V[140] = -0.07 
            V[40] = clamp(V[40] - 0.07, 0.3, 1) 
            V[135] = 3 
            V[138] = 25
        end
    elseif st == 3 then
        V[138] = V[138] - 1
        if slip > 2.5 and V[138] > 0 then
            V[140] = -0.06 
            V[40] = clamp(V[40] - 0.06, 0.3, 1) 
            V[135] = 1 
            V[136] = 0 
            V[139] = slip
        elseif V[138] <= 0 then 
    ...
    if V[135] == 0 then
        if slip < 1 then 
            V[40] = clamp(V[40] + (1 - V[40]) * 0.015, 0.3, 1) 
        else 
            V[40] = clamp(V[40] - slip * 0.0015, 0.3, 1) 
        end
    end
end
...
if th>120 and mph>minSpd and V[16]==0 and V[34]>0 and V[5]>(V[104]/100) and slip>tcSlip then raw=clamp((slip-tcSlip)*7,0,tcMax)end;if V[604]>0 and V[604]<4 then raw=m_min(raw,10)end;if V[624]>0 then raw=m_min(raw,5)end;if raw>0 then tcK=clamp(tcK+0.233135362,0,1)else tcK=tcK*0.995 end;tS=tcSlip;r0=raw
...
if V[133]>8 then local h=clamp((V[133]-8)/18,0,1);local runaway=(V[116]>0.78 and V[113]<0.20 and V[114]>0.60 and slip>tcSlip+3 and raw>V[36]*0.90);if not runaway then local rel=0.084735447*h+0.10*V[113]+0.06*(1-V[114]);if mph<8 then rel=rel+0.06*h end;V[36]=V[36]*(1-clamp(rel,0,0.45))end end
...
local function gm(g) return (g > 8 and g < 15) and g + ((g < 11 and 120) or (g == 11 and 123) or 128) or g
end

local function bb_line(g, dv, ev, gg, sc) local f = io.open("/LOGS/gvbb.csv", "a")
    if f then
        io.write(f, getTime(), ",", ev or 0, ",", gg or 0, ",", sc or m_floor(V[132] + .5), ",", g, ",", dv, "\n")
        pcall(io.close, f)
    end
end

local function xd(g, e, gps) if g == 125 then 
        return p2221((X[2] > 20 and X[1] < 20) and 2 or (X[2] > 0 and 3 or 0), X[1], X[2], X[3] / 10)
    end
    
    local rm = rpmMph(e)
    local md = (gps > 1 and rm > 1) and clamp(m_abs(rm - gps) / (gps + 3), 0, 1) or 0
    local dm = (V[1] == GPS_OK and 1) or (V[1] == GPS_COAST and 2) or (rm > 1 and 3 or 4)
    local pf = (gps > 90 or e > 60000 or md > .85 or X[5] > 25) and 1 or 0
    
    if g == 126 then 
        return p2221(dm, pf, (pf > 0 or X[7] == 0 or (V[620] > 0 and V[620] ~= 3) or dm == 4) and 1 or 0, 0)
    end
    
    if g == 127 then 
        return p2221(V[155] > 1 and 1 or 0, X[7], 0, V[168] / 10)
    end
    
    if g == 128 then 
        return p2221(V[370], V[336], V[545], V[339] / 10)
    end
    
    local ac = clamp(100 - V[222], 0, 99)
    local hr = clamp(V[625] * .45 + V[628] * .35 + (100 - (V[505] or 85)) * .20, 0, 99)
    
    return p2221(ac, V[346], hr, V[620] == 3 and 5 or (V[620] > 0 and 4 or (V[39] > 0 and 3 or (V[38] > 0 and 2 or (ac > 4 and 1 or 0)))))
end
