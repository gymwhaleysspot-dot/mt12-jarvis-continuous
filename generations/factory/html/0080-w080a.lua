-- factory full-AI cycle 80; html; gen 80; parent 0076-w076a.lua; objective memory efficiency, observability and deploy reliability
...
local POLE=2;local RPM2MPH=0.000931835;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function vl(n,d) local i=vi(n)
    if i and V[30]>5 and V[114]>.58 and X[7]>0 and V[1]==GPS_OK then
        local z=vs(i)
        vs(i,clamp(z+d,0,9))
        V[578]=n
        V[579]=d
...
local function voice(n,p,cd) p=p or 2
    cd=cd or 260
    local vq=clamp(vQuiet/50,1,2.6)
    if p<5 and vLoad>74 then voiceBlock=i9(voiceBlock); return end
    if p<4 and (V[550])>68 then voiceBlock=i9(voiceBlock); return end
    cd=m_floor(cd*vq+.5)
    if vcCd>0 and (p<vcPr or n==vcLast) then voiceBlock=i9(voiceBlock); return end
    local f=vf(n)
    if not f then voiceBlock=i9(voiceBlock); return end
    pcall(playFile,f)
    vcLast=n
    vcPr=p
    vcCd=cd
    ...
    V[572]=p
    V[573]=cd/10
    vLoad=clamp(vLoad+p*5,0,99)
    return n
end

local function voiceTick() if vcCd>0 then
        vcCd=vcCd-1
        if vcCd==0 then vcPr=0 end
    end
    vLoad=clamp(vLoad*.985,0,99)
    vQuiet=clamp(50+vLoad*.45+(V[550])*.20+(V[386])*.10,50,99)
    ...
    if vBoot==1 then
        ...
        vWait=42
    elseif vBoot==2 then
        voice((memLoaded>0) and 92 or 91,5,115)
        vBoot=3
        vWait=36
    elseif vBoot==3 then
        if memLoaded>0 and V[376]>0 then
            voice(93,5,95)
            pcall(playNumber,V[376],0)
        else
            voice(94,5,95)
        end
        vBoot=4
