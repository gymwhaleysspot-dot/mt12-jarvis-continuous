-- factory full-lineage cycle 29; monday; gen 29; experiment 3; parent a15xq.lua; objective truth prediction, calibration and sensor resilience
...
local POLE=2;local RPM2MPH=0.000919779;local WB=0.375;local MAX_STEER=26
local SURF={
...
if th>120 and mph>minSpd and V[16]==0 and V[34]>0 and V[5]>(V[104]/100) and slip>tcSlip then raw=clamp((slip-tcSlip)*7,0,tcMax)end;if V[604]>0 and V[604]<4 then raw=m_min(raw,10)end;if V[624]>0 then raw=m_min(raw,5)end;if raw>0 then tcK=clamp(tcK+.18,0,1)else tcK=tcK*0.979144092 end;tS=tcSlip;r0=raw
