-- factory full-lineage cycle 32; tuesday; gen 32; experiment 2; parent a15xq.lua; objective traction, ABS and intervention quality
...
if th>120 and mph>minSpd and V[16]==0 and V[34]>0 and V[5]>(V[104]/100) and slip>tcSlip then raw=clamp((slip-tcSlip)*7,0,tcMax)end;if V[604]>0 and V[604]<4 then raw=m_min(raw,10)end;if V[624]>0 then raw=m_min(raw,5)end;if raw>0 then tcK=clamp(tcK+0.229703015,0,1)else tcK=tcK*.985 end;tS=tcSlip;r0=raw
