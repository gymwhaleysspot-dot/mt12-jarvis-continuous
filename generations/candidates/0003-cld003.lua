--cld003 cloud LUAC Factory generation 3
-- style:confidence; independently synthesized MT12 controller
...
local function init()V[10]=0.000918;V[11]=22.385682;V[12]=21.174353;V[13]=325;V[14]=7223;V[15]=132;V[16]=472;V[17]=0.409212;V[18]=0.048818;V[19]=0.982105;V[20]=0.00231;end
local function sense()local g=val('GSpd',val('GPS',0)),r=val('RPM',0),s=val('Sats',0),rm=r*V[10],q=s>=6 and 1 or s>=5 and .82 or s>=4 and .5 or .15,d=rm-g;V[6]=V[6]+(q-V[6])*V[18];V[5]=V[5]+(d-V[5])*.08;local t=q>.72 and g+cl(d,-4,10)*(.25+q*.24) or rm>1 and rm*.98 or V[2]*V[19];V[2]=V[2]+(cl(t,0,120)-V[2])*V[17];V[35]=r-V[3];V[3]=r;return V[2],r,g,q,rm end
local function brain(m,r,g,q,rm)local th=val('thr',0),st=val('ste',0);V[30]=V[30]+(th-V[30])*.3;V[31]=V[31]+(st-V[31])*.28;V[34]=m-V[33];V[33]=m;local sl=max(0,rm-m),x=(sl-V[11])/V[12],tc=0,lk=0;if th>120 and m>1 then tc=cl(x*(.72+V[6]*.18),0,1)end;if th<-80 and m>4 then lk=cl((V[14]-r)/V[14]*.72+max(0,-V[35]/12000)*.28,0,1)end;local air=V[35]>3200 and V[34]<-1.2 and th>80;if air then tc=0 end;local unc=1-q,risk=cl(max(tc,lk)*.72+unc*.28+abs(V[34])*.006,0,1),tct=1024-floor(tc*V[13]);if q<.2 or air then tct=1024 end;local gyr=cl(V[15]+abs(st)*.05+tc*88-lk*24+abs(V[34])*2.2,80,V[16]);gv(7,tct);gv(1,gyr);gv(8,50+risk*40);if q>.8 and abs(th)<500 and r>1200 and g>4 then V[10]=cl(V[10]+(g/r-V[10])*V[20],.0008,.00105)end;V[70]=tc;V[71]=lk;V[72]=risk;V[73]=sl;V[74]=air and 1 or 0 end
