-- factory full-AI cycle 79; cloud; gen 79; parent 0076-w076a.lua; objective confidence arbitration, neural learning and reasoning
...
local POLE=2;local RPM2MPH=0.000936407;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function trait(old, score, rate) old = clamp(old or 50, 0, 99)
    ...
    local adaptiveRate = clamp(0.85 + (V[638] or 50) / 100, 0.95, 1.25)
    return clamp(old + (score - old) * (rate or 0.04) * adaptiveRate, 0, 99)
end

local function md(j, x, b) local d = x - (V[b] or x)
    local s = (V[j] - 50) * d > 0 and 1 or 0
    V[j] = trait(V[j], clamp(50 + d * 6, 0, 99), 0.65)
    V[b] = clamp((V[b] or x) * 0.65 + x * 0.35, 0, 99)
    return s
end
