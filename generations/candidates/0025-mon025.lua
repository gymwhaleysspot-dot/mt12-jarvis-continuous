-- factory full-lineage cycle 25; monday; gen 25; experiment 3; parent a15xq.lua; objective truth prediction, calibration and sensor resilience
...
local POLE=2;local RPM2MPH=0.000920235;local WB=0.375;local MAX_STEER=26
local SURF={
...
local sR=0;local cL=0;local srt=55;local clt=30;local sndCd=0;local sndPr=0;local sndLast=0;local metaLast=0;local vcCd=0;local vcLast=0;local vcCnt=0;local vcPr=0;local vBoot=1;local vWait=79;local vLoad=0;local vQuiet=50;local voiceBlock=0;local sndBlock=0
...
if th>120 and mph>minSpd and V[16]==0 and V[34]>0 and V[5]>(V[104]/100) and slip>tcSlip then raw=clamp((slip-tcSlip)*7,0,tcMax)end;if V[604]>0 and V[604]<4 then raw=m_min(raw,10)end;if V[624]>0 then raw=m_min(raw,5)end;if raw>0 then tcK=clamp(tcK+.18,0,1)else tcK=tcK*0.989855137 end;tS=tcSlip;r0=raw
