-- factory full-lineage cycle 33; cloud; gen 33; experiment 2; parent a15xq.lua; objective confidence arbitration, neural learning and reasoning
...
if th>120 and mph>minSpd and V[16]==0 and V[34]>0 and V[5]>(V[104]/100) and slip>tcSlip then raw=clamp((slip-tcSlip)*7,0,tcMax)end;if V[604]>0 and V[604]<4 then raw=m_min(raw,10)end;if V[624]>0 then raw=m_min(raw,5)end;if raw>0 then tcK=clamp(tcK+0.130986522,0,1)else tcK=tcK*0.990901273 end;tS=tcSlip;r0=raw
