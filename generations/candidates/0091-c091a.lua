-- factory full-AI cycle 91; cloud; gen 91; parent 0076-w076a.lua; objective confidence arbitration, neural learning and reasoning
...
local POLE=2;local RPM2MPH=0.000930697;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function memLearn(moving) if not moving then return end
    local gpsError = 1 - V[114]
    local terrainAdjustment = clamp(70 + V[116] * 20 - (V[113] * 15) - (V[36] > 25 and 10 or 0), 0, 100)
    local responseTime = clamp(70 + (V[113] + V[119] + gpsError) * 10, 0, 100)
    local gpsPrecision = clamp(V[40] * 85, 0, 100)
    local accelerationBias = clamp(70 + V[117] * 20 - (V[37] > 25 and 10 or 0), 0, 100)

    V[127] = V[127] + (terrainAdjustment - V[127]) * 0.02
    V[128] = V[128] + (responseTime - V[128]) * 0.02
    V[129] = V[129] + (gpsPrecision - V[129]) * 0.015
    V[130] = V[130] + (accelerationBias - V[130]) * 0.015

    if sR > 0.5 then srt = clamp(srt + (((V[16] > 0) and 50 or 60) - srt) * 0.008, 45, 70) end
    clt = clamp(clt + (((cL > 0.6) and 25 or 35) - clt) * 0.005, 20, 50)
