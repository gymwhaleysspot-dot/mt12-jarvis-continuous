-- factory full-AI cycle 89; monday; gen 89; parent 0076-w076a.lua; objective truth prediction, calibration and sensor resilience
...
local POLE=2;local RPM2MPH=0.000929996;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function bb_tick(gps,e,rx,sats,rq) if not alive(gps,e,rx,sats,rq) then vG=1; vGT=0; bc=0; return end
    bm=V[650]; bc=bc+1; if bc < ((bm==0 and 3) or (bm==1 and 2) or 1) then return end; bc=0

    if unsafePending > 0 then 
        bb_line(108, p2221(V[588], V[589], V[590], V[593]), 0)
        bb_line(109, p2221(V[591], V[592], V[168], V[169]), 0)
        unsafePending = 0 
    end

    if V[594] > 0 then 
        bb_line(116, V[594], 0); V[594] = 0 
    end

    if X[24] > 0 then 
        bb_line(130, X[24], 0); bb_line(131, X[25], 0); X[24] = 0; X[25] = 0 
    end

    if X[29] > 0 then 
        bb_line(135, X[29], 0)
        if X[30] > 0 then bb_line(136, X[30], 0) end
        X[29] = 0; X[30] = 0 
    end

    local ev = (V[38] > 0 and 1 or 0) + (V[39] > 0 and 2 or 0) + (V[23] > 0 and 4 or 0) + (V[16] > 0 and 8 or 0) + 
                (((V[41] > 0 or V[43] > 0 or V[119] > .48) and 16 or 0)) + 
                ((V[1] ~= GPS_OK) and 32 or 0) + ((V[620] == 3) and 64 or 0) + bm * 128

    local ck = (V[39] > 0 and V[38] > 0 and 3) or (V[39] > 0 and 2) or (V[38] > 0 and 1) or 0
    if ck > 0 then 
        if evK == 0 then 
            evK = ck; evDur = 0; evPeak = 0; evEntry = V[155]; evReason = V[223]; evMin = V[35]; evTC = 0; evABS = 0; V[679] = V[652] 
        end
        if ck == 3 then evK = 3 end
        evDur = evDur + 1; evPeak = m_max(evPeak, 100 - V[35]); evMin = m_min(evMin, V[35]); 
        evTC = m_max(evTC, V[225] or 0); evABS = m_max(evABS, V[226] or 0)
        
        if V[223] > 0 and V[223] ~= 9 then evReason = V[223] 
        elseif evK > 0 then 
            local rc = (V[35] > 90 and evMin > 78 and V[119] < .65) and 1 or 0
            V[346] = clamp(100 - evPeak * 2 - evDur * .7 + (rc > 0 and 18 or 0), 0, 99)
            bb_line(80, p2221(evK, evDur, evPeak, evReason), ev)
            bb_line(81, p2221(evEntry, V[35], rc, evMin / 10), ev)
            bb_line(85, p2221(evTC, evABS, rc, evMin / 10), ev)
            V[289] = V[289] + 1
            if evK == 1 or evK == 3 then V[549] = V[549] + 1 end
            if evK == 2 or evK == 3 then V[552] = V[552] + 1 end
            if rc == 0 then V[553] = V[553] + 1 end
            V[554] = evK; V[555] = evPeak; V[556] = evDur; V[557] = rc; evK = 0; evTC = 0; evABS = 0 
        end
    end

    local gg = vG * 10 + (vGT > 0 and 1 or 0)
    local sc = m_floor(V[132] + .5); dI = dI % 24 + 1
    local dG

    if bm > 1 then 
        bi = bi % 9 + 1
        dG = gm(string.byte("\061\062\075\057\011\013\106\008\046\075\057\061\011\008\106\013\046\067\075\057\061\008\106\013\046\074\011\058\059\061\057\075\013\008\067\011\123\124\075\061\057\126\011\013\008\123\124\061\075\057\011\013\008\046\125\126\127\046\074\057\011\013\106\061\062\009\010\011\106\013\057\075", (V[652] - 1) * 9 + bi)) 
    else 
        dG = gm(string.byte("\046\057\058\059\061\062\067\074\075\095\106\123\124\125\126\127\009\010\011\012\013\014\008\001", dI)) 
    end

    local dV = 0
    if dG == 46 then 
        dV = p332(V[155] * 10, rpmMph(e), V[182]) 
    elseif dG == 57 then 
        dV = p2221(V[113] * 100, V[116] * 100, V[117] * 100, V[119] * 10) 
    elseif dG == 58 then 
        dV = p332(V[30], V[169], gv(6)) 
    elseif dG == 59 then 
        dV = p2221(V[70], V[120], vG, V[6]) 
    elseif dG == 61 then 
        dV = p2221(V[180], V[181], V[182], V[183] / 10) 
    elseif dG == 62 then 
        dV = p2221(V[184], V[185], V[187], V[186] / 10) 
    elseif dG == 1 then 
        dV = p332(pD, eT, dT) 
    elseif dG == 67 then 
        local sm = SG[V[186]] or {0, 50, 0, 0, 0, 0, 0}; 
        dV = p2221(sm[1], sm[5], sm[6], (sm[7] or 0) / 10) 
    elseif dG == 74 then 
        dV = p332(e / 100, V[100], V[101]) 
    elseif dG == 75 then 
        dV = p2221(V[35], V[223], V[225], V[226] / 10) 
    elseif dG == 95 then 
        dV = p2221(V[373], V[375], V[376], memLoaded) 
    elseif dG == 106 then 
        dV = p2221(V[570], vLoad, voiceBlock, sndBlock / 10) 
    elseif dG == 123 then 
        dV = p2221(V[423], V[424], V[620], V[6]) 
    elseif dG == 124 then 
        dV = p2221(V[409], V[410], V[418], V[604]) 
    elseif dG >= 125 and dG <= 129 then 
        dV = xd(dG, e, gps) 
    elseif dG == 132 then 
        dV = p2221(X[31], X[34], m_min(X[32], X[35]), V[6]) 
    elseif dG == 139 then 
        dV = p2221(V[570], V[659], V[660], V[661] / 10) 
    elseif dG == 140 then 
        dV = p2221(V[650], V[652], V[658], V[670 + V[652]] / 10) 
    elseif dG == 141 then 
        dV = p2221(V[680], V[681], V[682], V[683] / 10) 
    else 
        dV = p2221(V[119] * 100, V[117] * 100, sR * 100, cL * 10) 
    end
