-- factory full-lineage cycle 30; tuesday; gen 30; experiment 3; parent a15xq.lua; objective traction, ABS and intervention quality
...
if th>120 and mph>minSpd and V[16]==0 and V[34]>0 and V[5]>(V[104]/100) and slip>tcSlip then raw=clamp((slip-tcSlip)*7,0,tcMax)end;if V[604]>0 and V[604]<4 then raw=m_min(raw,10)end;if V[624]>0 then raw=m_min(raw,5)end;if raw>0 then tcK=clamp(tcK+0.237243323,0,1)else tcK=tcK*.985 end;tS=tcSlip;r0=raw
...
if V[133]>8 then local h=clamp((V[133]-8)/18,0,1);local runaway=(V[116]>0.78 and V[113]<0.20 and V[114]>0.60 and slip>tcSlip+3 and raw>V[36]*0.90);if not runaway then local rel=0.079328529*h+0.10*V[113]+0.06*(1-V[114]);if mph<8 then rel=rel+0.06*h end;V[36]=V[36]*(1-clamp(rel,0,0.45))end end
