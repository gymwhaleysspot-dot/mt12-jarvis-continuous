-- factory full-AI cycle 87; cloud; gen 87; parent 0076-w076a.lua; objective confidence arbitration, neural learning and reasoning
...
local POLE=2;local RPM2MPH=0.000935636;local WB=0.375;local MAX_STEER=26
local SURF={
...
local function condBrain(moving,mph,e,rM,th,thDelta,st,dSt,brake,brakeLoad,slip,tcCut,absCut,md,btl,rq,sats,brR,spR,hz,good,scare) if not moving then return end
 if V[502] == 0 or btl > V[502] then V[502] = btl end
 local lag = clamp((V[502] or btl) - btl, 0, 99)
 V[501] = clamp(V[501] * 0.95 + lag * 0.05, 0, 99)
 V[505] = clamp((V[505] or btl) * 0.99 + btl * 0.01 - (th > 250 and 0.002 or 0.0005), 0, 99)
 V[500] = clamp((V[500] or 85) * 0.99 + (100 - (V[501]) * 2) * 0.01, 0, 99)
 V[503] = clamp((V[503] or 0) * 0.995 + clamp((V[505] or btl) - btl, 0, 30) * 0.02 + (V[262] or 0) * 0.01 - (V[262] < 8 and 0.05 or 0), 0, 99)
 V[504] = clamp((V[505] or btl) * 0.4 + (rq or 0) * 0.2 + (sats >= 5 and 20 or 5) - (V[501]) * 0.3, 0, 99)
 local rear = clamp((slip * mph) * 0.1 + (tcCut or 0) * 0.6 + ((th > 450) and 8 or 0) + (V[38] > 0 and 8 or 0), 0, 99)
 local front = clamp((absCut or 0) * 1.1 + brR * 40 + brakeLoad * 20 + m_abs(st) / 1024 * 15 + (V[39] > 0 and 8 or 0), 0, 99)
 V[510] = clamp((V[510] or 20) + ((rear - (V[510] or 20)) * ((rear > V[510]) and 0.03 or 0.008)), 0, 99)
 V[511] = clamp((V[511] or 20) + ((front - (V[511] or 20)) * ((front > V[511]) and 0.03 or 0.01)), 0, 99)
 local tireAvg = ((V[510]) + (V[511])) * 0.5
 V[512] = clamp(100 - m_abs(tireAvg - 50) * 1.2 - (V[513]) * 0.2, 0, 99)
 V[513] = clamp((V[513]) * 0.98 + clamp(tireAvg - 60, 0, 40) * 0.05 + ((tcCut or 0) + (absCut or 0)) * 0.01, 0, 99)
 local jit = clamp(m_abs(dSt) / 500 + m_abs(thDelta) / 800 + (brake and m_abs(thDelta) / 900 or 0), 0, 1)
 local evt = ((tcCut or 0) > 2 or (absCut or 0) > 2) and 1 or 0
 V[520] = clamp((V[520] or 20) * 0.98 + (jit * 50 + evt * 15 + (V[343] or 0) * 0.5 + hz * 15) * 0.01, 0, 99)
 V[521] = clamp(100 - (V[520]) * 0.7 - (V[204]) * 0.2, 0, 99)
 V[522] = clamp((V[522]) * 0.99 + ((V[300]) > 900 and V[520] > 40 and 0.1 or 0) - ((good > 0 and V[520] < 20) and 0.03 or 0), 0, 99)
 V[523] = clamp(100 - (V[520]) * 0.5 - (V[522]) * 0.3 - (V[342]) * 0.6, 0, 99)
 V[530] = clamp((V[347] or 80) * 0.4 + (V[512] or 80) * 0.2 + (V[521] or 80) * 0.15 + (good > 0 and 15 or 0) - scare * 10 - md * 15, 0, 99)
 V[531] = clamp((V[531] or 50) + (V[530] - 50) * 0.001 + (((tcCut or 0) > 2 and good > 0) and 0.01 or 0), 0, 99)
 V[532] = clamp((V[532] or 50) + (V[530] - 50) * 0.001 + (((absCut or 0) > 1 and brake) and 0.01 or 0), 0, 99)
 V[533] = clamp((V[303] or 100) * 0.25 + (V[530]) * 0.3 + (V[521]) * 0.15 + (V[500] or 80) * 0.1 - (V[513]) * 0.15, 0, 99)
 V[534] = (V[503] > 30 and 3) or (V[513] > 25 and 4) or (V[520] > 50 and 5) or (V[530] > 75 and 2) or 1
 if V[503] > 40 then V[339] = clamp((V[339] or 50) - 0.003, 35, 65) V[340] = clamp((V[340] or 50) - 0.002, 35, 65) end
 if V[513] > 30 then V[337] = clamp((V[337] or 50) + 0.003, 35, 65) V[338] = clamp((V[338] or 50) + 0.002, 35, 65) end
 if V[520] > 50 then V[336] = clamp((V[336] or 50) - 0.004, 35, 65) V[361] = ((V[361] or 1) == 9) and 2 or V[361] end
end
