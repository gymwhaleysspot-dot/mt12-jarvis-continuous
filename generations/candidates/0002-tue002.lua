--tue002 tuesday LUAC Factory generation 2
...
local function init()V[10]=0.000909;V[11]=21.819214;V[12]=21.921601;V[13]=301;V[14]=6961;V[15]=113;V[16]=472;V[17]=0.474999;V[18]=0.052425;V[19]=0.982242;V[20]=0.001962;end
...
local function brain(m,r,g,q,rm)local th=val('thr',0),st=val('ste',0);V[30]=V[30]+(th-V[30])*.3;V[31]=V[31]+(st-V[31])*.28;V[34]=m-V[33];V[33]=m;local sl=max(0,rm-m),x=(sl-V[11])/V[12],tc=0,lk=0;if th>120 and m>1 then tc=cl(x*.48+max(0,V[34])*.012+x*x*.18,0,1)end;if th<-80 and m>4 then lk=cl((V[14]-r)/V[14]*.72+max(0,-V[35]/12000)*.28,0,1)end;local air=V[35]>3200 and V[34]<-1.2 and th>80;if air then tc=0 end;local unc=1-q,risk=cl(max(tc,lk)*.78+abs(V[34])*.01+abs(V[35])/18000,0,1),tct=1024-floor(tc*V[13]);if q<.2 or air then tct=1024 end;local gyr=cl(V[15]+abs(st)*.05+tc*88-lk*24+abs(V[34])*2.2,80,V[16]);gv(7,tct);gv(1,gyr);gv(8,50+risk*40);if q>.8 and abs(th)<500 and r>1200 and g>4 then V[10]=cl(V[10]+(g/r-V[10])*V[20],.0008,.00105)end;V[70]=tc;V[71]=lk;V[72]=risk;V[73]=sl;V[74]=air and 1 or 0 end
