-- factory full-lineage cycle 47; tuesday; gen 47; experiment 2; parent 0042-m0422.lua; objective traction, ABS and intervention quality
...
local POLE=2;local RPM2MPH=0.00091792;local WB=0.375;local MAX_STEER=26
local SURF={
...
if V[133]>8 then local h=clamp((V[133]-8)/18,0,1);local runaway=(V[116]>0.78 and V[113]<0.20 and V[114]>0.60 and slip>tcSlip+3 and raw>V[36]*0.90);if not runaway then local rel=0.129230679*h+0.10*V[113]+0.06*(1-V[114]);if mph<8 then rel=rel+0.06*h end;V[36]=V[36]*(1-clamp(rel,0,0.45))end end
